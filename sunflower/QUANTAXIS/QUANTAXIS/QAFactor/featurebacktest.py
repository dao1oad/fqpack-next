...
class QAFeatureBacktest:
    def __init__(
        ...
        ).apply(lambda x: self.slice_feature(x))
        self.datacenter = self.client.get_stock_day_qfq_adv(
            self.codelist, self.start, self.end
        )
        self.closepanel = (
            self.datacenter.closepanel.bfill()
        )  ## 向前复权 匹配股票停牌模式 使用复牌后第一个收盘价卖出
        runtime_mongo_ip = QA_util_resolve_mongo_runtime(mongo_ip or qaenv_mongo_ip).uri
        self.account = QIFI_Account(
            init_cash=init_cash,
            username='QAFB_{}_{}'.format(self.featurename, uuid4()),
            broker_name='feature',
            portfolioname=portfolioname,
            password='1',
            nodatabase=False,
            model='BACKTEST',
            trade_host=runtime_mongo_ip,
        )
        self.tradetable: dict[Any, dict[str, int]] = {}
    ...
    def get_sell_list(self, date):
        ...
        try:
            sell = list(
                self.tradetable[QA_util_get_last_day(date, self.rolling - 1)].keys()
            )
            return sell
        except:
            return []
    ...
    def run(
        ...
        for date in QA_util_get_trade_range(self.start, self.end):
            ...
            if len(selllist) == 0:
                ...
                data = (
                    self.closepanel.loc[parser.parse(date).date(), selllist]
                ...
                for code in selllist:
                    ...
                    volume = self.tradetable[
                        QA_util_get_last_day(date, self.rolling - 1)
                    ][code]
                    if volume < 100:
                        pass
                    else:
            ...
            if len(buylist) != 0:
                ...
                d = self.datacenter.selects(buylist, date, date).open.map(
                    lambda x: round(x, 2)
                )
                ...
                for code in buylist:
                    try:
                        volume = (
                            int(0.01 * cashpre / data[code]) * 100
                            if data[code] != 0
                            else 0
                        )
                        if volume < 100:
            ...
            pricepanel = self.closepanel.loc[
                parser.parse(date).date(), holdinglist
            ].map(lambda x: round(x, 2))
