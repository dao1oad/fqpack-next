...
class QA_QIFIMANAGER:
    ...
    def __init__(
        self, account_cookie, mongo_ip=None, database='quantaxis', collection='history'
    ):
        self.mongo_uri = QA_util_resolve_mongo_runtime(mongo_ip or qaenv_mongo_ip).uri
        self.database = self.change_database(database, collection)
        self.database.create_index(
            [("account_cookie", pymongo.ASCENDING), ("trading_day", pymongo.ASCENDING)],
            unique=True,
        )
    ...
    def get_qifislice(self, date):
        return self.database.find_one(
            {'account_cookie': self.account_cookie, 'trading_day': date}
        )
    ...
    def month_assets_profit(self):
        res = (
            pd.concat([pd.Series(self.assets.iloc[0]), self.month_assets])
            .diff()
            .dropna()
        )
    ...
    def get_historyassets(
        self, start='1990-01-01', end=str(datetime.date.today())
    ) -> pd.Series:
        b = [
            (item['accounts']['balance'], item['trading_day'])
            for item in self.database.find(
                {'account_cookie': self.account_cookie},
                {'_id': 0, 'accounts': 1, 'trading_day': 1},
            )
        ]
        ...
        res = (
            res.assign(
                ...
                balance=res.balance.apply(round, 2),
            )
            .dropna()
            .set_index('datetime')
            .sort_index()
    ...
    def get_historytrade(
        ...
        res = (
            res.assign(
                account_cookie=res['user_id'],
                code=res['instrument_id'],
                tradetime=res['trade_date_time'].apply(
                    lambda x: datetime.datetime.fromtimestamp(x / 1000000000)
                ),
            )
            .set_index(['tradetime', 'code'])
            .sort_index()
        )
    ...
    def get_sharpe(self):
        ...
        a = ((n.iloc[-1] / n.iloc[0] - 1) / len(n) * 365) / abs(
            (n.pct_change() * 100).std()
        )
        return 0 if np.isnan(a) else a

    def show_perf_stats(self, live_start_date=None):
        pf.show_perf_stats(
            self.returns, self.benchmark_returns, live_start_date=live_start_date
        )
...
class QA_QIFISMANAGER:
    ...
    def __init__(self, mongo_ip=None, account_cookie='', model='BACKTEST'):
        self.mongo_uri = QA_util_resolve_mongo_runtime(mongo_ip or qaenv_mongo_ip).uri

        if model == 'REALTIME':
            self.database = pymongo.MongoClient(self.mongo_uri).QAREALTIME.account
        else:

            self.database = pymongo.MongoClient(self.mongo_uri).quantaxis.history
            self.database.create_index(
                [
                    ("account_cookie", pymongo.ASCENDING),
                    ("trading_day", pymongo.ASCENDING),
                ],
                unique=True,
            )
    ...
    def get_allportfolio(self) -> list:
        ...
        return list(
            set(
                [
                    i['portfolio']
                    for i in self.database.find({}, {'portfolio': 1, '_id': 0})
                ]
            )
        )
            ...
            set(
                [
                    ...
                    for i in self.database.find(
                        {'portfolio': portfolio}, {'account_cookie': 1, '_id': 0}
                    )
                ]
            )
        )
        ...
        return list(
            set(
                [
                    i['account_cookie']
                    for i in self.database.find(
                        {'account_cookie': {"$regex": text}},
                        {'account_cookie': 1, '_id': 0},
                    )
                ]
            )
        )
    ...
    def get_portfolio_panel(self, portfolio) -> pd.DataFrame:
        ...
        rp = [
            self.database.find_one(
                {'account_cookie': i}, {"accounts": 1, 'trading_day': 1, '_id': 0}
            )
            for i in r
        ]
        return pd.DataFrame(
            [mergex(i['accounts'], {'trading_day': i['trading_day']}) for i in rp]
        ).query('user_id in {}'.format(r))
    ...
    def get_allaccountname(self) -> list:
        return list(
            set(
                [
                    i['account_cookie']
                    for i in self.database.find({}, {'account_cookie': 1, '_id': 0})
                ]
            )
        )

    def get_historyassets(
        self, account_cookie, start='1990-01-01', end=str(datetime.date.today())
    ) -> pd.Series:
        b = [
            (item['accounts']['balance'], item['trading_day'])
            for item in self.database.find(
    ...
    def get_portfolio_assets(
        self, portfolio, start='1990-01-01', end=str(datetime.date.today())
    ) -> pd.Series:
        """
        ...
        """
        return pd.concat(
            [self.get_historyassets(acc, start, end) for acc in portfolio], axis=1
        )
    ...
    def get_historyorders(self, account_cookie):
        ...
        res = (
            res.assign(
                account_cookie=res['user_id'],
                code=res['instrument_id'],
                ordertime=res['insert_date_time'].apply(
                    lambda x: datetime.datetime.fromtimestamp(x / 1000000000)
                ),
            )
            .set_index(['ordertime', 'code'])
            .sort_index()
        )
        return res.drop_duplicates().sort_index()
    ...
    def rankstrategy(self, code):
        ...
        rp = (
            res.apply(self.get_sharpe)
            + res.tail(50).apply(self.get_sharpe)
            + res.tail(10).apply(self.get_sharpe)
        ).sort_values()
    ...
    def get_historypos(self, account_cookie):
        b = [
            mergex(
                list(item['positions'].values())[0],
                {'trading_day': item['trading_day']},
            )
            for item in self.database.find(
                {'account_cookie': account_cookie},
                {'_id': 0, 'positions': 1, 'trading_day': 1},
            )
        ]
        res = pd.DataFrame(b)
        res.name = account_cookie
    ...
    def get_historymargin(self, account_cookie):
        b = [
            (item['accounts']['margin'], item['trading_day'])
            for item in self.database.find(
                {'account_cookie': account_cookie},
                {'_id': 0, 'accounts': 1, 'trading_day': 1},
            )
        ...
        res = pd.DataFrame(b, columns=['balance', 'trading_day'])
        res = (
            res.assign(datetime=pd.to_datetime(res['trading_day']))
            .set_index('datetime')
            .sort_index()
        )
        res = res.balance
    ...
    def get_holding_panel(self, account_cookie, trading_day):
        ...
        b = list(
            self.database.find_one(
                {'account_cookie': account_cookie, 'trading_day': trading_day},
                {'_id': 0, 'positions': 1},
            )['positions'].values()
        )
        res = pd.DataFrame(b)
    ...
    def get_holding_block(self, account_cookie, trading_day):
        b = list(
            self.database.find_one(
                {'account_cookie': account_cookie}, {'_id': 0, 'positions': 1}
            )['positions'].values()
        )
        res = pd.DataFrame(b)
        res.name = account_cookie
