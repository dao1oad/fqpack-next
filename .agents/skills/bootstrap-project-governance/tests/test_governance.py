...
class GovernanceFixture(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.repo.mkdir()
        self.run_process(
            [sys.executable, "-X", "utf8", str(BOOTSTRAP), "apply", "--repo", str(self.repo), "--project-name", "测试 <项目>"],
            check=True,
        )
    ...
    def gov(self, *args: str, input_value: dict | None = None) -> subprocess.CompletedProcess[str]:
        return self.run_process(
            [sys.executable, "-X", "utf8", str(self.tool), *args, "--repo", str(self.repo)],
            cwd=self.repo,
    ...
    def configure(
        ...
        *,
        ...
        project["fallbacks"] = [{"when": "external_unready", "action": "continue_independent_slices"}]
...
class GovernanceRuntimeTests(GovernanceFixture):
    def test_installer_is_idempotent_and_preserves_hook(self) -> None:
        bootstrap_skill = self.repo / ".agents" / "skills" / "bootstrap-project-governance" / "SKILL.md"
        ...
        hooks["hooks"]["SessionStart"] = [{"hooks": [{"type": "command", "command": "echo existing"}]}]
        hooks_path.write_text(json.dumps(hooks, indent=2) + "\n", encoding="utf-8")
        second = self.run_process(
            [sys.executable, "-X", "utf8", str(BOOTSTRAP), "apply", "--repo", str(self.repo)], check=True
        )
        ...
        stop_handlers = [
            ...
            if "governance.py" in f"{handler.get('command', '')} {handler.get('commandWindows', '')}"
        ]
        ...
        devin_hooks["SessionStart"] = [{"hooks": [{"type": "command", "command": "echo existing"}]}]
        devin_hooks_path.write_text(json.dumps(devin_hooks, indent=2) + "\n", encoding="utf-8")
        ...
        checked = self.run_process(
            [sys.executable, "-X", "utf8", str(BOOTSTRAP), "check", "--repo", str(self.repo)]
        )
        self.assertEqual(checked.returncode, 0, checked.stderr + checked.stdout)
        if os.name == "nt":
            self.assertTrue(devin_stop_handlers[0]["command"].startswith("powershell.exe "))
    ...
    def test_ready_start_hook_run_and_complete(self) -> None:
        ...
        implemented = self.gov("record", "--type", "WORK_IMPLEMENTED", "--item", "WI-001")
    ...
    def test_source_change_makes_evidence_stale(self) -> None:
        self.start()
        self.assertEqual(self.gov("run", "--item", "WI-001", "--gate", "e2e-real").returncode, 0)
    ...
    def test_host_adapter_change_preserves_evidence_protocol(self) -> None:
        self.start()
        self.assertEqual(self.gov("run", "--item", "WI-001", "--gate", "e2e-real").returncode, 0)
        self.assertEqual(self.gov("record", "--type", "WORK_IMPLEMENTED", "--item", "WI-001").returncode, 0)
        self.tool.write_text(self.tool.read_text(encoding="utf-8") + "\n# host adapter metadata\n", encoding="utf-8")
        report = json.loads(self.gov("check", "--completion").stdout)
        self.assertTrue(report["eligible"], report)
        self.assertEqual(report["items"][0]["checks"][0]["status"], "passed")
