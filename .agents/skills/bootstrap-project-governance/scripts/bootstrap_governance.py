...
BOOTSTRAP_SKILL_MARKER = (
    "Install a small repository-local governance runtime shared by Codex and Devin"
)
...
ATTRIBUTES_BEGIN = "# BEGIN BOOTSTRAP-PROJECT-GOVERNANCE"
ATTRIBUTES_END = "# END BOOTSTRAP-PROJECT-GOVERNANCE"
ATTRIBUTES_BODY = """/.governance/*.json text eol=lf
/.governance/*.jsonl text eol=lf
/.governance/runs/**/result.json text eol=lf
/.governance/runs/**/stdout.log -text
/.governance/runs/**/stderr.log -text"""
IGNORE_BEGIN = "# BEGIN BOOTSTRAP-PROJECT-GOVERNANCE"
IGNORE_END = "# END BOOTSTRAP-PROJECT-GOVERNANCE"
IGNORE_BODY = """!.governance/runs/**/stdout.log
!.governance/runs/**/stderr.log"""
...
def handler_is_governance(handler: Any, host: str) -> bool:
    if not isinstance(handler, dict):
        return False
    field = "commandWindows" if host == "codex" and os.name == "nt" else "command"
    expected = governance_hook_group(host)["hooks"][0]
    return handler.get("type") == "command" and handler.get(field) == expected.get(
        field
    )


def handler_mentions_governance(handler: Any) -> bool:
    if not isinstance(handler, dict):
        return False
...
def group_has_expected_matcher(
    group: dict[str, Any], expected_group: dict[str, Any]
) -> bool:
    return ("matcher" in group) == ("matcher" in expected_group) and group.get(
        "matcher"
    ) == expected_group.get("matcher")
...
def has_governance_hook(config: dict[str, Any], host: str) -> bool:
    ...
    for group in groups:
        ...
        for handler in handlers if isinstance(handlers, list) else []:
            ...
            if (
                handler_is_governance(handler, host)
                and handler == expected_handler
                and group_has_expected_matcher(group, expected_group)
            ):
                return True
    return False
...
def merge_hooks(path: Path, host: str) -> str:
    if path.exists():
        ...
        config = (
            {"description": "Project lifecycle hooks.", "hooks": {}}
            if host == "codex"
            else {}
        )
    if host == "codex":
    ...
    for group in stop:
        ...
        for index, handler in enumerate(handlers if isinstance(handlers, list) else []):
            if not handler_mentions_governance(handler):
                continue
            found = True
            if handler == expected_handler and group_has_expected_matcher(
                group, expected_group
            ):
                return "preserve"
            handlers[index] = expected_handler
            if isinstance(group, dict):
                if "matcher" in expected_group:
                    group["matcher"] = expected_group["matcher"]
                else:
...
def merge_agents(path: Path) -> str:
    ...
    if AGENTS_BEGIN in text and AGENTS_END in text:
        pattern = re.compile(
            re.escape(AGENTS_BEGIN) + r".*?" + re.escape(AGENTS_END), re.DOTALL
        )
...
def managed_text_block(begin: str, body: str, end: str) -> str:
    return f"{begin}\n{body.strip()}\n{end}"


def managed_text_action(path: Path, begin: str, end: str) -> str:
    if not path.exists():
        return "create"
    text = path.read_text(encoding="utf-8", errors="replace")
...
def merge_managed_text(path: Path, *, begin: str, body: str, end: str) -> str:
    block = managed_text_block(begin, body, end)
    action = managed_text_action(path, begin, end)
    if action == "create":
        path.write_text(block + "\n", encoding="utf-8", newline="\n")
        return action
    text = path.read_text(encoding="utf-8")
...
def has_managed_text_block(path: Path, begin: str, body: str, end: str) -> bool:
    if not path.is_file():
        return False
    text = path.read_text(encoding="utf-8", errors="replace").replace("\r\n", "\n")
    return managed_text_block(begin, body, end) in text
...
def managed_skill(path: Path, marker: str) -> bool:
    skill = path / "SKILL.md"
    return skill.is_file() and marker in skill.read_text(
...
def build_plan(repo: Path, project_name: str, upgrade: bool) -> dict[str, Any]:
    ...
    for relative in (
        ...
        operations.append(
            {
                "path": relative,
                "action": "preserve" if (repo / relative).exists() else "create",
            }
        )

    for relative, host in (
        (".codex/hooks.json", "codex"),
        (".devin/hooks.v1.json", "devin"),
    ):
        ...
        if hooks_path.exists():
            try:
                ...
                hook_action = (
                    "preserve" if has_governance_hook(hooks, host) else "merge"
                )
    ...
    operations.append(
        {"path": "AGENTS.md", "action": agents_action(repo / "AGENTS.md")}
    )
    operations.extend(
        [
            {
                "path": ".gitattributes",
                "action": managed_text_action(
                    repo / ".gitattributes", ATTRIBUTES_BEGIN, ATTRIBUTES_END
                ),
            },
            {
                "path": ".gitignore",
                "action": managed_text_action(
                    repo / ".gitignore", IGNORE_BEGIN, IGNORE_END
                ),
            },
        ]
    )
    operations.append(
        {
            ...
            "action": project_skill_action(
                repo, BOOTSTRAP_SKILL_REL, BOOTSTRAP_SKILL_MARKER, upgrade
            ),
        }
    )
    operations.append(
        {
            ...
            "action": project_skill_action(
                repo, GRILLING_SKILL_REL, GRILLING_SKILL_MARKER, upgrade
            ),
        }
    )
    return {
...
def apply(repo: Path, project_name: str, upgrade: bool) -> dict[str, Any]:
    ...
    if not tool_target.exists() or (upgrade and managed_tool(tool_target)):
        ...
        applied.append(
            {
                "path": "tools/governance.py",
                "action": "update" if tool_existed else "create",
            }
        )

    template_pairs = (
        (
            ASSET_ROOT / ".governance" / "project.json.template",
            repo / ".governance" / "project.json",
        ),
        (
            ASSET_ROOT / ".governance" / "work.json.template",
            repo / ".governance" / "work.json",
        ),
        (
            ASSET_ROOT / ".governance" / "events.jsonl.empty",
            repo / ".governance" / "events.jsonl",
        ),
    )
    for source, target in template_pairs:
        ...
        applied.append(
            {"path": target.relative_to(repo).as_posix(), "action": "create"}
        )
    ...
    attributes_action = merge_managed_text(
        repo / ".gitattributes",
        begin=ATTRIBUTES_BEGIN,
        body=ATTRIBUTES_BODY,
        end=ATTRIBUTES_END,
    )
    applied.append({"path": ".gitattributes", "action": attributes_action})
    ignore_action = merge_managed_text(
        repo / ".gitignore",
        begin=IGNORE_BEGIN,
        body=IGNORE_BODY,
        end=IGNORE_END,
    )
    applied.append({"path": ".gitignore", "action": ignore_action})
    bootstrap_skill_action = project_skill_action(
        repo, BOOTSTRAP_SKILL_REL, BOOTSTRAP_SKILL_MARKER, upgrade
    )
    if bootstrap_skill_action in {"create", "update"}:
        copy_bootstrap_skill(repo / BOOTSTRAP_SKILL_REL)
    applied.append(
        {"path": BOOTSTRAP_SKILL_REL.as_posix(), "action": bootstrap_skill_action}
    )
    grilling_skill_action = project_skill_action(
        repo, GRILLING_SKILL_REL, GRILLING_SKILL_MARKER, upgrade
    )
    if grilling_skill_action in {"create", "update"}:
        copy_grilling_skill(repo / GRILLING_SKILL_REL)
    applied.append(
        {"path": GRILLING_SKILL_REL.as_posix(), "action": grilling_skill_action}
    )
    return {
        "applied": applied,
        "repo": str(repo),
        "next": [
            "edit project.json and work.json",
            "review hook trust",
            "governance.py ready",
            "governance.py start",
        ],
...
def check_install(repo: Path) -> dict[str, Any]:
    ...
    for governance_rel in (
        ...
        if not (repo / governance_rel).is_file():
            issues.append(f"缺少 {governance_rel}")
    for hook_rel, host, label in (
    ...
    if not has_managed_text_block(
        repo / ".gitattributes", ATTRIBUTES_BEGIN, ATTRIBUTES_BODY, ATTRIBUTES_END
    ):
        issues.append(".gitattributes 缺少治理证据行尾托管区块")
    if not has_managed_text_block(
        repo / ".gitignore", IGNORE_BEGIN, IGNORE_BODY, IGNORE_END
    ):
        issues.append(".gitignore 缺少治理日志例外托管区块")
    for skill_rel, marker, label in (
        (
            BOOTSTRAP_SKILL_REL,
            BOOTSTRAP_SKILL_MARKER,
            "项目级 bootstrap-project-governance Skill",
        ),
        (GRILLING_SKILL_REL, GRILLING_SKILL_MARKER, "项目级 grilling Skill"),
    ):
        if not managed_skill(repo / skill_rel, marker):
