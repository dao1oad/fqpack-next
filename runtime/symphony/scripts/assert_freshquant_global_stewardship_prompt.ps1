...
$requiredPatterns = @(
    ...
    @{ Name = 'origin main refetch rule'; Pattern = 'Fetch latest `origin/main` before the round starts and fetch latest `origin/main` again after closing any covered issue before deciding the next batch' },
    @{ Name = 'deploy plan script rule'; Pattern = 'freshquant_deploy_plan\.py' },
    @{ Name = 'handoff candidate recompute rule'; Pattern = 'Treat the merge handoff packet as candidate input only;\s*recompute the effective deploy batch against current `main` for every round' },
    ...
    @{ Name = 'proxyless health check rule'; Pattern = 'Use proxyless localhost health checks for `127\.0\.0\.1` and `localhost`;\s*do not trust ambient `HTTP_PROXY`, `HTTPS_PROXY`, or `ALL_PROXY` when checking fixed ports' },
    ...
    @{ Name = 'timeout result review rule'; Pattern = 'If a deploy command or runtime-control command times out,\s*enter a result-review branch and inspect compose metadata,\s*container state,\s*image identity,\s*logs,\s*and health evidence before marking the batch failed' },
    @{ Name = 'subagent evidence only rule'; Pattern = 'Subagents may collect GitHub,\s*deploy,\s*runtime,\s*health,\s*frontend,\s*and cleanup evidence,\s*but only the parent steward may decide deploy,\s*follow-up issue creation,\s*cleanup completion,\s*or issue closure' },
