...
$requiredPatterns = @(
    ...
    @{ Name = 'post-approval no reapproval rule'; Pattern = 'Once Design Review is approved, do not ask for new human approval to handle CI, merge conflicts, deploy failures, or cleanup failures within the same issue scope\.\s*Route that work to `Rework` or `Merging` by default;\s*use `Blocked` only when a new real external blocker appears' },
