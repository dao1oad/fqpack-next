...
$requiredPatterns = @(
    ...
    @{ Name = 'merge handoff state rule'; Pattern = '-\s+`Merging`:\s+perform one-shot GitHub truth check,\s+merge the PR,\s+write the structured merge handoff packet,\s+and move the issue to `Global Stewardship`\.' },
    ...
    @{ Name = 'handoff candidate rule'; Pattern = 'The `Merging` handoff packet is candidate input only;\s*do not claim runtime delivery or cleanup success from it\.' },
