...
- 目标：仅做 **本机安装 + 使用手册**，**不**把 `symphony` 正式接入本项目运行链
...
如果未来想把 `Symphony` 作为 FreshQuant 的正式编排能力纳入项目：

- 必须先写 RFC
- 必须明确它与当前 `AGENTS.md` 治理规则的关系
- 必须明确 workspace 模型、分支策略、CI 门禁、回滚策略
...
在 FreshQuant 中试用 `Symphony` 前，需要先知道这些边界：

- **冲突 1：工作区模型**
  - 上游 reference implementation 默认是 issue workspace/repo copy
  - 本项目要求 `git worktree + feature branch`

- **冲突 2：无人值守执行**
  - 上游设计目标之一是 unattended Codex
  - 本项目大量任务要求先界定需求边界、先写 RFC、再编码

- **冲突 3：外部依赖**
  - 实际使用 `linear` tracker 时，会新增对 Linear 工作流的依赖
  - 对本项目而言，这已经不是单纯“工具安装”问题，而是治理和集成问题

因此，当前最合理的结论是：

- 本次安装结果可用于 **本机评估**
- 不能把“已安装成功”理解成“已经适合直接用于本项目正式开发流程”
