...
- `docs/migration/progress.md` 中 `0001` 到 `0027` 当前均为 `Done`
- `0028` 当前处于 `Implementing`：正在将仓库治理从 `worktree-first + reviewer-first` 切换为 `Linear-first + Symphony-first + design-approval-first`
- 已落地核心能力包括：
...
**用途**：说明 FreshQuant 如何把 `Symphony` 作为正式开发编排器接入，包括 Linear 状态机、唯一人工门、分支策略、默认 `subagent + TDD` 与自动化权限边界。
