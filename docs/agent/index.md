...
- `docs/migration/progress.md` 中 `0001` 到 `0030`、`0032` 当前已 `Done`；`0031` 正在 `Implementing`
- `0028` 当前已 `Done`：仓库治理已切换为 `Linear-first + Symphony-first + design-approval-first`，并将 `CD` 固化到 `Merging -> Done` 语义中
- `0033` 当前已 `Done`：`Done` 真值进一步收紧为 `merge + deploy + health check + cleanup`，并补齐 `远端 feature branch / Symphony workspace / 旧 artifacts` 三类 cleanup 硬门禁
- 已落地核心能力包括：
  - XTData producer/consumer
  - ETF `qfq`
  - 订单管理 / 仓位管理 / TPSL / 信用账户订单
  - Gantt / Shouban30 读模型与页面（含 `GanttHistory` 图表层审计对齐）
  - KlineSlim 多周期缠论图层与结构面板
...
**用途**：说明如何在 Windows 本机安装 OpenAI `Symphony` 官方 Elixir 参考实现，并给出本机验证方式。正式治理请另看下方“Symphony 正式接入治理说明”。

**适用场景**：

- 本机验证 `Symphony` 是否可运行
- 需要 `memory` tracker 的最小 smoke test
- 对照正式治理文档理解本机安装与正式运行的边界

---

### [Symphony正式接入治理说明](./Symphony正式接入治理说明.md)
`symphony-formal-governance-guide`

**用途**：说明 FreshQuant 如何把 `Symphony` 作为正式开发编排器接入，包括 Linear 状态机、唯一人工门、分支策略、默认 `subagent + TDD`、`Merging` 自动部署与自动化权限边界。

**适用场景**：

- 理解仓库为什么从 `AGENTS + worktree-first` 切到 `Symphony-first`
- 设计/评审正式 `Linear` 工作流
- 实际接入前核对治理边界

---

### [Symphony宿主机服务部署说明](./Symphony宿主机服务部署说明.md)
`symphony-host-service-deployment-guide`

**用途**：说明如何在 Windows 宿主机上以正式服务方式运行 Symphony，包括运行目录、同步脚本、启动脚本、`NSSM` 安装方式与日志路径。

**适用场景**：

- 准备把 Symphony 从演示脚本升级为正式常驻编排器
- 需要在当前账号下安装 `fq-symphony-orchestrator`
- 需要升级、重启或回滚宿主机 Symphony 服务

---

### [Symphony真实Issue阻塞复盘与优化建议](./Symphony真实Issue阻塞复盘与优化建议.md)
`symphony-fre-5-postmortem`

**用途**：复盘真实 issue `FRE-5` 在 Symphony 正式接入中的多次阻塞、最终成功条件，以及建议固化的配置和流程优化。

**适用场景**：

- 理解为什么真实 issue 会在 Windows 宿主机服务模式下多次阻塞
- 需要区分“运行面问题”和“业务代码问题”
- 准备加固 Symphony 正式运行面时参考已有经验
...
| 理解 Symphony 正式治理 | [Symphony正式接入治理说明](./Symphony正式接入治理说明.md) |
| 部署 Symphony 宿主机服务 | [Symphony宿主机服务部署说明](./Symphony宿主机服务部署说明.md) |
| 复盘 Symphony 真实 issue 阻塞与优化 | [Symphony真实Issue阻塞复盘与优化建议](./Symphony真实Issue阻塞复盘与优化建议.md) |
