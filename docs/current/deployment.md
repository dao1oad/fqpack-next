...
`audit --mode structure` 检查日期覆盖、唯一键、正因子和末日因子，并读取 XTData instrument `OpenDate` / `IsTrading` 元数据区分上市前覆盖与 `nontrading_terminal_history`；它还要求 slot `source_exclusions[]` 中的 code 没有因子残留，但不加载 source bars。正式发布 gate 使用 `--mode full` 从 XTData 重新加载 source bars 并验证完整递推恒等式；excluded code 仍为空才通过，若 bars 已恢复则要求重建。`--mode tail` 可配合 `--code` 做低成本定位。
