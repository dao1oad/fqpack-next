...
`audit --mode structure` 检查日期覆盖、唯一键、正因子和末日因子，并读取 XTData instrument `OpenDate` / `IsTrading` 元数据区分上市前覆盖与 `nontrading_terminal_history`；它还要求 slot `source_exclusions[]` 中的 code 没有因子残留，但不加载 source bars。full-scope audit 会覆盖 metadata 中全部 exclusion code，即使已离开当前 universe；`--code` 只审请求交集。正式发布 gate 使用 `--mode full` 从 XTData 重新加载 source bars 并验证完整递推恒等式；excluded code 只有重现 metadata 中的 `source_empty_bars`、`source_adjustment_gap_unproven` 或 `source_prefix_unavailable` 相同 reason 才通过，缺少 expected history、source 恢复、gap 可证明或 reason 改变均结构化标记 stale 并令 audit 失败。`--mode tail` 可配合 `--code` 做低成本定位。
...
CLX 浏览器 smoke 以 `/daily-screening?tab=clx` master-detail 工作台为主；另用带旧 query 的 `/clx-daily-screening` 检查兼容 redirect，最终地址应收敛到 `/daily-screening?tab=clx` 并保留可消费的 scope、条件和线关系状态，不应重新挂载独立筛选页。
