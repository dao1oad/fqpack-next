...
- 组合总览的账户净资产曲线按 QMT 口径计算：单位净值 =（基金资产总值 − 基金负债）/ 基金总份额，
  ...
  **曲线按日采样**（北京日历日桶取当日末笔快照），并**复用交易日历缓存**
  （`trade_calendar_cache`，market=cn_a/source=sina）过滤非交易日：X 轴不含
  周末/节假日（如 2026-05-01），点间直接连线无异常横线；交易日历不可用或
  过期时降级为按日历日展示并附 `trade_calendar_unavailable` 告警。窗口以
  最新快照为锚往前截取、
  ...
  `sleep(15s)`，100 实测相邻 `queried_at` 间隔中位 15.1s），采样接近全天候
  （每日约 5,600 条），周末与法定节假日同样产生快照——因此 X 轴必须按交易日历
  过滤，而不是依赖"有快照即交易日"。
- 组合总览 KPI 的"账户净资产" = 总资产 − 总负债（QMT 口径），来源优先级与
  `net_value_source` 对应：①券商快照自带 `total_debt` → `broker_asset_minus_debt`；
  ②券商快照无负债字段（信用账户现状）→ 总资产/净资产/现金整体切换最新信用资产
  快照口径（`credit_snapshot_reconstructed`，算术自洽）；③两者都不可用 → 降级
  net=总资产（`broker_total_asset` / `credit_asset_fallback`）。修复前券商分支
  曾把净资产误算为总资产（未减负债）。
  同图叠加基准曲线 = 上证综指ETF 510210（`quantaxis.index_day` 日线，紫色虚线，
  缺失分桶沿用上一收盘价），**净资产与基准归一化到同一 Y 轴**（各自可见区间
  首交易日=100；半年/一年/两年窗口首日即 2026-04-07），面板徽章显示区间
  涨跌幅与跑赢/跑输幅度；
  交易发生的周期在图上标注交易点，悬浮展示该周期全部交易的时间、标的、方向、数量、
  价格、金额、请求 ID、关联质量与账户分区，并汇总当日买入/卖出合计金额。
- 组合总览悬浮框统一深色实底主题（`#0f172a` 底、正文 `#f1f5f9`、次级
  `#94a3b8`，对比度满足 WCAG AA）：普通点显示日期、净资产/总资产（区间涨幅）、
  较前一交易日涨跌、持仓市值、现金、总负债，有基准时追加基准收盘价（区间涨幅）
  与"相对基准 ±Xpp 跑赢/跑输"。
- 交易点交互：**悬浮**显示资产快照卡并在卡内合并"当日成交明细"（日期、笔数、
  当日买卖合计）；**点击**交易标识弹出自绘固定成交卡，卡内按**订单聚合**展示
  （标的 × 请求 × 方向：成交数量、加权均价、金额、成交笔数、请求ID、触发信号、
  关联质量、账户分区，不列逐笔成交），并附当日买入/卖出汇总统计（订单数、
  合计金额与股数）；右上角 × 或点击空白处关闭。
  固定卡打开期间暂停 axis 悬浮卡（避免双卡同屏）；卡片空白区域不拦截图表
  悬浮/点击（pointer-events 只作用于内容与关闭按钮）。请求 ID 与券商委托 ID
  双空的成交不跨成交合并（各成一行），无复盘匹配的成交信号显式标"证据缺失"，
  无价格的成交不计入加权均价分母；较前一交易日取前一个有效交易日值。
  注意：ECharts 全局 `trigger:'axis'` 下 series 级 item tooltip 不渲染，因此
  固定卡为页面层自绘，不依赖 ECharts tooltip。
- 交易日集合在 `PositionReviewService` 内做 600s 进程内缓存；kline-slim 订单
  复盘悬浮框统一使用同一深色主题常量；`/stock-position-list` 深链重定向到首页。
...
- KlineSlim 的“交易复盘”覆盖层同样消费 `/symbols/<symbol>/chart` 只读投影并在价格层渲染 marker，使用相同的完整悬浮框；覆盖层默认展示 B/S 买卖标识（进入行情图即自动加载，顶栏“交易复盘”开关仅用于隐藏/再显示），已移除“持仓均价”阶梯线（不再绘制 `order-review-chart-cost` 系列，后端 `cost_basis_series` 字段继续返回给复盘页证据消费）；不再在 K 线下方绘制策略应有量/实际成交量/连续持仓三轨附图，旧 `/timeline` 接口已移除。
