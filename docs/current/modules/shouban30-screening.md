...
- Docker 并行部署下，`fq_apiserver` 当前必须挂载 `${FQPACK_TDX_SYNC_DIR:-D:/new_tdx}` 到 `/opt/tdx`，并以 `FRESHQUANT_TDX__HOME=/opt/tdx` 读取；Compose 插值变量必须在调用进程中设置，不能依赖 service `env_file`
