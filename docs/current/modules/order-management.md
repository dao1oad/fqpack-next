...
- `om_entry_slices`
  ...
  - `arrange_entry` 按 `price × grid_interval` 乘法递增、单格股数
    由 `trading.board_lot` 整手 helper（根⑤，步骤 7：科创板 200 股、其余
    100 股整手；科创板按上交所规则 ≥200 股 + 1 股递增）切分；当整手金额已
    超过 `lot_amount`（价格过高、网格不再有意义的细分粒度）时，剩余量全部
    并入最后一格，保证 `Σslice == entry` 守恒、价格有界且终止（避免低价
...
- `#582` 收口：守恒守门另含**归属一致性**检查
  ...
  底仓等错配会被探针与对账告警自动捕获
- `#588`：归属一致性对共享账户**镜像机**场景豁免——本地无 request 的 broker-only
  成员若 broker order 携带 OM 提交 token（`FQOM` 前缀），跳过（该订单由真实下单机
  经 OrderManagement 提交，意图证据在对方机）；提交机（request 存在）路径不受影响，
  错标仍必报；broker order 侧 token 缺失时回退到 `om_orders.broker_correlation_token`
  再判定（历史幽灵写入可能抹掉 broker order 侧 token，#588 加固）
