...
`/api/order-management/orders` 当前对 `date_from / date_to` 的无时区输入按北京时间解释；页面列表、详情时间轴与成交时间也统一换算为北京时间并显示到秒。
...
- 顶部筛选标签、订单摘要条和右侧详情 identifier chip 当前统一复用共享 `StatusChip`，不再由页面单独维护 muted / warning / success pill 语义
