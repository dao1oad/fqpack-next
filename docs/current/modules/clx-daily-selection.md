...
同一交易日如果本侧 marker 版本改变，新的 `selection_key` 形成新的 batch generation；旧 partition 保留为不可变历史，不会被原地覆盖。finalizer 只 join 当前两侧 selection key 对应的输出。
...
partition 在计算前后各读取一次本侧 ready marker，并与规划时冻结的 `marker_snapshot_hash` 对比。任一阶段发生变化时，本侧 attempt 结束为 `upstream_drift`，本次计算结果不提交；另一侧的不可变成功输出保持可复用。

finalizer 读取当前 marker 时，任一侧 marker 缺失会把该侧投影为 `status=waiting / upstream_status=marker_missing`，而不是抛出异常或复用旧 generation。若同交易日 marker 已更新，当前 selection key 会形成新的 batch generation；旧 generation 的 failed/pending final 内容保留审计，但不会继续 publication。
...
- finalizer job 必须携带 `trade_date / batch_id / partition_ids / finalization_attempt_id` tags。job 先按 attempt id 读取持久化计划，再逐项强校验 tag 与持久化的 trade date、batch 和两个不可变 partition id；不一致直接失败，不根据运行时 tag 临时改写计划。
...
- publication 身份固定为 `generation_id / generation_order / publication_id`。`generation_order` 把两侧 marker 的最新 `document_updated_at` 规范成 UTC 微秒键 `YYYY-MM-DDTHH:mm:ss.ffffffZ|batch_id`，避免 `Z` 与 `+00:00` 的字符串格式差异破坏排序；相同 `publication_id` 的重试幂等成功。若新 generation 已先写入 ready marker，迟到旧 publisher 会收到 `stale_publication`，旧 batch 的 publication 保持 `failed` 并记录结构化 `last_error`，不会被推进为 `published`。
...
`/history/signals` 当前只接受 `period=1d`，`barCount` 范围为 `1..2000`。`endDate` 可显式传入；省略时由 provider 解析该标的最新交易日。响应包含 bar 等长的模型序列、压缩 marker、bar 等长的 `line_series`（当前正式计算 MA250，其他未接入连线保持 unknown）、`calculation_profile`、`future_function_guard`、`input_bar_asof`、实际 `end_date` 和 `query_hash`；HTTP 响应用 `query_hash` 生成 ETag。
...
- 查 sensor run tag 中的 `fq_clx_selection_key / fq_clx_attempt_no`。
...
- 核对 Dagster tags 的 `fq_trade_date / fq_clx_batch_id / fq_clx_partition_ids / fq_clx_finalization_attempt_id` 与持久化 attempt 完全一致。
...
- 若 sensor 规划后 marker 版本或当前 partition id 改变，执行结果为 `generation_drift`，本次 `finalization_attempt` 记为 failed。
...
- 比较 attempt 中冻结的 `marker_snapshot_hash` 与当前本侧 ready marker。
- 确认上游是否在计算期间覆盖了同交易日 marker 的 `run_id / updated_at / payload`。
- 只重试发生漂移的一侧；另一侧 completed partition 保持不变。
