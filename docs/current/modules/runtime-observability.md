...
- 原先右上角的视图切换、自动刷新、仅异常、高级筛选与刷新按钮，当前统一挪到左上标题区，与时间区间框并排
...
- `标的名称` 优先读 trace 顶层 `symbol_name` 与 step 内联名称字段；默认不再在 `/api/runtime/traces` 热路径里同步查询 instrument 信息
- 只有显式传 `include_symbol_name=1` 调 `/api/runtime/traces` 或 `/api/runtime/traces/<trace_id>` 时，后端才会按需回填缺失的 `symbol_name`
- 右侧详情区统一改成 tab 语法：
  - Trace：`步骤 / 摘要 / 原始数据`
  - Event：`事件 / 载荷 / 原始数据`
...
- 左侧组件切换只有在当前视图是 `组件 Event` 时才会触发 Event 重载；停留在 `全局 Trace` 视图时不再为了预取 Event 额外卡顿
