...
没有最新 health 数据的组件不会消失，而是返回占位项：`status=unknown`、`heartbeat_age_s=null`、`is_placeholder=true`，页面显示为 `unknown / no data`。组件详细健康、异常链路数、异常节点数和 highlights 现在在左侧窄栏中直接以高密度台账展开，而不是 hover popover。
