...
`Guardian / 止盈开关切换 -> subject-management / tpsl 接口 -> 图表价格横线重绘`

`Guardian / 止盈全部开启/关闭 -> 配置保存 + Guardian state / takeprofit rearm -> 图表价格横线重绘`
...
- `klineSlimSidebar.mjs`
  - 页面壳级 sidebar bridge
  - 对 `js/kline-slim-sidebar.mjs` 的 sidebar section 结果再做一层页面字段归一，保证 `titleLabel / secondaryLabel` 两行摘要语义在页面层稳定
- `klineSlimPageState.mjs`
  - 页面壳级路由 symbol、当前周期与 overlay panel 互斥状态 helper
  - 当前统一承接 `showPriceGuidePanel / showSubjectPanel / showChanlunStructurePanel` 的互斥关闭语义
- `kline-slim.js`
  - 页面状态、路由、侧边栏、主图/多周期数据请求
  - 当前通过 `klineSlimSidebar.mjs` 和 `klineSlimPageState.mjs` 接入页面壳级 sidebar / page-state bridge
...
- `kline-slim-chart-controller.mjs`
  ...
  - `Shift + 左键拖拽` 会在主图 grid 内同步平移 `X/Y` 两轴；普通拖拽仍沿用现有横轴平移语义，不与画线拖拽冲突
  - `yMode=auto` 时，Y 轴默认只根据当前可见主图 candles / 笔段 / 结构框 auto-fit；仅 `画线编辑` 模式或主图值为空时才把 Guardian / 止盈价格线并入 Y 轴范围
  - 成本价线与买入订单线默认会并入 `auto` Y 轴范围；若在 legend 中关闭，对应价格线也会同时退出 Y 轴计算
  - 第一次滚轮缩放进入 `manual` 时，会先按缩放后的可见主图重算 Y 基线，再按鼠标价格锚点同步缩放，避免远离主图的价格线把 Y 轴长期撑宽
  - 通过 `graphic` overlay 命中可编辑价格线，按当前 grid + viewport 做像素/价格坐标换算；图上拖拽取价当前按三位小数处理
- `kline-slim-chart-renderer.mjs`
  - 负责主 K 线、多周期笔/段/高级段图层以及结构框 overlay
  - 负责 Guardian / 止盈 / 成本价 / 买入订单价格横线 overlay，并在 legend 中提供默认开启的独立开关
...
- `subject-price-guides.mjs`
  - 统一归一化 Guardian / 止盈草稿、成本价、open buy lot 价格以及价格线 / legend 分组生成逻辑
  - 统一提供价格草稿回填、顺序夹紧（Guardian `BUY-1 > BUY-2 > BUY-3`、止盈 `L1 < L2 < L3`）和编辑态横线数据

当前 legend 语义是：

- legend 控制主周期缠论层和额外周期叠加层
- legend 同时控制 Guardian / 止盈 / 成本价 / 买入订单价格横线分组，默认显示
...
- `/kline-big`、`/multi-period`、`/kline-slim` 当前在根容器层统一接入共享 `workbench-page` 视口壳；图表内部仍保留各自的 KlineHeader、overlay 和深色主题语法
- 周期按钮切换
- `1m / 5m / 15m / 30m / 1d` 周期
- 历史日期回看
- 缠论结构面板
...
- `must_pool` 基础配置当前统一编辑止损价、首笔金额、常规金额和“单标的上限设置”；分类继续沿用当前真值，`forever` 固定写 `true`，页面不再暴露这两个项
- `基础配置` 区当前采用 2 列卡片布局；区块摘要统一展示 `当前止损 / 当前上限 / 市值 / 允许买入|已阻断买入`
- 工具栏状态条、`标的设置` 浮层摘要、`画线编辑` 浮层摘要和 `缠论结构` 摘要行当前统一复用共享 `StatusChip` 语义；页面本地只保留布局类，不再重复维护 `.status-chip / .price-panel-summary-chip / .chanlun-summary-chip` 配色
- 单标的仓位上限不再单独占一个卡片；输入框默认回填当前生效值 `effective_limit`，卡片内部只保留标题、当前生效值和输入框
- 若保存值与系统默认值相同，后端会自动删除 symbol override；浮层不再提供 `use_default / 恢复默认 / 默认-单独切换`
- `画线编辑` 头部按钮当前为 `保存`，只保存 Guardian / 止盈 6 个价格值，不改运行态
- `Guardian 倍量价格` 与 `止盈价格` 区域各自提供 `全部开启 / 全部关闭`，批量同步三层 `开/关` 配置和运行态
- Guardian `buy_active` 与止盈 `armed_levels` 不提供逐层手动设置入口；前端只在分区批量按钮触发时把运行态整体对齐到当前配置
- `Guardian 倍量价格` 与 `止盈价格` 区块头部都会显示运行态汇总 `运行态 X/3`
- `止盈价格` 区块的 header、摘要区和行内布局当前与 `Guardian 倍量价格` 保持一致；批量按钮固定横排显示
- `画线编辑` 浮层内，`止盈价格` 区块当前位于 `Guardian 倍量价格` 上方，以对应主图上更高位置的价格线
...
- 图上默认直接显示成本价横线与当前 open buy lot 买入价横线；两类新增横线都只读，不进入拖拽编辑
- `画线编辑` 模式下可直接拖拽 Guardian / 止盈价格横线设置价格，拖拽结束自动保存
- `画线编辑` 的 Guardian / 止盈价格输入、拖拽草稿、图上标签、十字准星价格和保存回写当前统一按小数点后三位处理
- `Shift + 左键拖拽` 可同时平移 X/Y 轴；`重置视图` 可回到默认 `auto viewport`
- 工具栏当前只保留 `画线编辑` 作为价格面板入口；不再单独提供 `价格层级` 按钮，浮层标题也统一为 `画线编辑`
- `重置视图` 会把当前图表从手动双轴缩放恢复到默认 `auto viewport`
- 图例控制主图缠论层、额外周期叠加和价格横线分组；新增 `成本价线` 与 `买入订单线` legend 开关
...
- 标的设置浮层头部只保留 `保存/关闭` 动作；`按 buy lot 止损` 区不再直接裸露长 `buy_lot_id`，而是展示“第 N 笔买入”中文摘要、买入时间、当前 `buy_price_real`、当前系统仓位市值真值和剩余百分比，并把原始 ID 降级为辅助标签
- 缠论结构面板以覆盖在主图上的浮层显示
- `画线编辑`、`标的设置` 与 `缠论结构` 按钮都支持再次点击关闭；三个大面板当前按互斥方式打开，避免彼此重叠
- 多个侧边栏分组统一使用两行紧凑布局：第一行显示 `名称(代码)`，第二行显示单条摘要信息
- `持仓股` 分组第二行只显示仓位金额，展示口径与 `/subject-management` 运行态列一致；不再显示持仓股数
- `持仓股` 分组默认按仓位金额降序展示，排序口径使用 `position_amount -> market_value -> amount`；页面未传 `symbol` 时，默认打开该排序后的第一只持仓
...
- 检查 legend 里 `Guardian 价格线` / `止盈价格线` / `成本价线` / `买入订单线` 是否被手动关闭
- 检查 `画线编辑` 按钮是否被再次点击关闭；当前行为应为 toggle 而不是单向打开
- 如果是 `全部开启 / 全部关闭` 后图上线条样式没变化，检查对应运行态是否已随批量按钮同步更新，而不是只改了配置开关
...
- 若感觉只能按两位小数拖拽，检查控制器的像素转价格换算和十字准星格式化是否仍停留在两位小数
