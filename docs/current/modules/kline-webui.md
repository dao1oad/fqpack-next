...
`/kline-slim` 负责单标的多周期图表、缠论结构和单笔止损浮层。当前与订单账本相关的部分已经切到 `position entry` 语义。
...
- 单笔止损浮层中的止损对象已经是 open `entries`
- 保存止损时只提交 `entry_id`
- 浮层详情返回 `entries`，不再依赖 `buy_lots`
- 单笔止损浮层只保留 `按持仓入口止损` 工作区；不再在图表页暴露 `must_pool`、止损价、首笔金额、常规金额和单标的仓位上限输入区
- 单笔止损浮层里的 entry 止损摘要当前显示买入价、原始数量、剩余数量与比例、该笔剩余市值以及买入时间；为节省横向空间，买入股数和剩余股数展示为 `万股`，且买入价不再显示 `买入价:` 标签
- 单笔止损浮层里的每个 entry 整行 hover 都会显示 `切片明细`；浮层只展示当前 entry 的 open slices，不增加额外按钮，也不展开 `aggregation_members`
...
- `symbol -> /api/subject-management/<symbol>`
  - 读取单笔止损浮层 detail，其中仍带 `mustPool` / `positionLimit` 只读信息
- `保存单笔止损`
  - `entry stoploss bind`
- `保存画线编辑`
  - Guardian 配置
