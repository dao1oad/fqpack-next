...
`Guardian / 止盈保存 -> subject-management / tpsl 接口 -> 图表价格横线与带状网格重绘`
...
- `subject-price-guides.mjs`
  - 统一归一化 Guardian / 止盈草稿、校验规则以及价格线/价格带生成逻辑
...
- 检查 Guardian / 止盈价格线是否已经并入 `priceGuideLines` / `priceGuideBands`
...
- 检查 renderer scene 是否带上了 `priceGuideLines` / `priceGuideBands`
