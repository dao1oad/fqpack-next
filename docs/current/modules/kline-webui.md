...
- 标的设置浮层里的 entry 止损摘要当前固定显示为两行标签化摘要：第一行是 `买入价：26.655；买入5000 股 剩 5000 股 / 100.00%`，第二行是 `买入时间：2026-03-30 10:47:36；剩余市值：13.33 万`；其中买入价固定保留 3 位小数，剩余市值固定按“万”为单位保留 2 位小数，关键数值在弹层里按标签/数值分组高亮
