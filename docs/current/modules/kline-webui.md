...
`symbol -> /api/subject-management/<symbol> -> 价格层级浮层 + 标的设置浮层`

`Guardian / 止盈保存 -> subject-management / tpsl 接口 -> 图表价格横线重绘`

`保存并激活 -> guardian-buy-grid + guardian_buy_grid_state + takeprofit save + takeprofit rearm -> 图表价格横线重绘`
...
- `kline-slim.js`
  ...
  - 价格层级面板的 symbol 级加载、保存和重绘触发；首屏进入 `/kline-slim?symbol=...` 时就会立刻请求 `/api/subject-management/<symbol>`
  - 标的设置浮层的 symbol 级加载、保存和 stoploss 行内更新
  - `画线编辑` 模式切换、拖拽草稿更新和拖拽结束自动保存
- `kline-slim-price-panel.mjs`
  - Guardian / 止盈草稿状态、保存校验、保存并激活编排与接口动作封装
...
- 价格层级浮层头部不再提供独立刷新按钮，Guardian / 止盈区也不再展示“蓝 / 红 / 绿实/虚线”等说明文字
- 标的设置面板也以覆盖在主图上的浮层显示，不再挤占图表布局
- 标的设置浮层头部只保留保存/关闭动作；`按 buy lot 止损` 区不再直接裸露长 `buy_lot_id`，而是展示“第 N 笔买入”中文摘要、买入时间/价格/原始数量/剩余数量，并把原始 ID 降级为辅助标签
- 缠论结构面板以覆盖在主图上的浮层显示
- `价格层级`、`标的设置` 与 `缠论结构` 按钮都支持再次点击关闭；三个大面板当前按互斥方式打开，避免彼此重叠
- 多个侧边栏分组；`持仓股` 分组当前额外显示仓位金额和持仓数量摘要，展示口径与 `/subject-management` 运行态列一致
- 热门理由 hover 展示
...
- 检查 `价格层级` 按钮是否被再次点击关闭；当前行为应为 toggle 而不是单向打开
...
- 检查 `subjectDetailLoading` / `savingPriceGuideActivation` / `savingGuardianPriceGuides` / `savingTakeprofitGuides` 是否使编辑态被锁定
