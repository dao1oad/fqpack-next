...
- `must_pool` 基础配置当前只编辑分类、止损价、首笔金额和常规金额；`forever` 固定写 `true`，页面不再暴露该项
...
- 标的设置浮层头部使用紧凑摘要布局；`按 buy lot 止损` 区不再直接裸露长 `buy_lot_id`，而是展示“第 N 笔买入”中文摘要、买入时间/价格/原始数量/剩余数量，并把原始 ID 降级为辅助标签
- 缠论结构面板以覆盖在主图上的浮层显示
