...
- “单标的仓位上限覆盖”里的“订单推断仓位 / stock_fills 仓位”当前改成弹性宽列，会优先吃掉操作列后的剩余宽度，并收紧行高与来源文本间距，表头与行密度对齐 `/runtime-observability` 的 dense ledger
- “当前命中规则”卡片当前复用 `position-metric-card` 的标题与数值样式，只保留详情文案段落，和“可用保证金”等指标卡保持同一视觉语法
