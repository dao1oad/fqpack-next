...
- “单标的仓位上限覆盖”固定放在右栏，集中展示 `系统默认值 / 单标的上限设置 / 当前来源 / 一致性 / 门禁 / 操作 / 券商同步仓位 / 订单推断仓位 / stock_fills 仓位`
...
- “单标的仓位上限覆盖”里的 `券商仓位 / 订单推断仓位 / stock_fills 仓位` 当前都挪到“操作”列后，并改成固定宽度列；来源文本会截断显示并保留 tooltip，避免扰乱主列排版
