...
- Memory 热库默认 `fq_memory`
- 冷记忆目录默认 `.codex/memory`
- context pack 产物根目录默认 `D:/fqpack/runtime/symphony-service/artifacts/memory`
...
`FQ_MEMORY_CONTEXT_PATH` 由 `runtime/symphony/scripts/run_freshquant_codex_session.ps1` 在会话启动前注入，指向本轮编译好的 memory context pack。它只作为派生上下文，不是正式真值源。
`FQ_MEMORY_CONTEXT_ROLE` 由同一 wrapper 同步注入，用于标识当前会话读取的是 `codex` 还是 `global-stewardship` context pack。
