...
- `source_empty_bars_excluded > 0` 表示完整 `none` source 区间下载后仍为空；从 active slot `source_exclusions[]` 核对 code/reason。该 code 在对应 slot 必须没有因子行，不会填 `1.0`；structure audit 检查无残留，tail/full audit 重新按完整区间读取。source 恢复后 audit 会报告 `stale_source_exclusion/rebuild_required`，应重建 inactive slot；prefix/suffix、proof 或公司行为错误不进入此分类。
