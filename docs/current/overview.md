...
- HTTP API 已统一挂载 `stock / gantt / daily-screening / clx-backtest / order / position-management / position-review / subject-management / tpsl / runtime / system-config`
- CLX 大规模回测已形成 `不可变快照 -> 因果逐日前缀 -> 事件研究 -> VALIDATION 排行冻结 -> 一次 HOLDOUT 揭示 -> 组合撮合 -> API / Web UI` 的独立研究链
...
- `freshquant/`
  ...
  - `backtest/clx` 承担 CLX 快照、因果信号、排行与组合撮合
...
- 保持 CLX artifact 血缘、冻结顺序和 HOLDOUT 单次读取边界可验证
