...
- `morningglory/fqwebui` 当前正式前端 gate 已收口到 `lint + test:browser-smoke + test:unit + build`，并同时接入 `.github/workflows/ci.yml` 与 `script/fq_local_preflight.ps1`；其中 `test:browser-smoke` 当前会实际执行 Playwright Chromium smoke spec，`test:unit` 默认跳过 7 个已登记的 known-red Node 用例，完整集合保留在 `test:unit:all`。
