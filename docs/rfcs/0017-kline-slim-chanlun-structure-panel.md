...
- [ ] 后端服务测试能固定最后一个高级段、段、笔的方向和端点提取语义
- [ ] 后端服务测试能固定中枢归属和包含数量统计语义
- [ ] 路由测试能验证 `/api/stock_data_chanlun_structure` 调用新服务并返回 JSON
- [ ] `KlineSlim` 页面存在 `缠论结构` 按钮
- [ ] 打开面板后能看到 `高级段 / 段 / 笔` 三个区块
- [ ] 面板具备 `刷新 / 关闭 / 重试` 交互
- [ ] 实时模式可基于 Redis bar 重建 `fullcalc` 结构
- [ ] 历史模式可基于 `symbol + period + endDate` 返回同一结构语义
...
- M1：RFC 0017 起草并进入 Implementing
