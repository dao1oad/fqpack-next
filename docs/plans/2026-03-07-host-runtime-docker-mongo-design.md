...
`envs.conf` 仅设置了 `FRESHQUANT_MONGODB__HOST=127.0.0.1`，未设置 `FRESHQUANT_MONGODB__PORT`，因此这些宿主机进程默认连接宿主机 `127.0.0.1:27017`。

与此同时，Docker 并行部署的 Mongo 运行在 `fq_mongodb`，宿主机映射端口是 `27027`。容器内 API 已连接 Docker Mongo，但宿主机进程仍连接宿主机 Mongo，导致 `params / xt_positions / xt_trades` 等运行数据分裂。

## 目标

- 让宿主机 `broker / producer / consumer` 统一连接 Docker Mongo `127.0.0.1:27027`
...
这样通过 supervisor 启动的三个宿主机进程会统一改连 Docker Mongo。
...
- 将 `D:/fqpack/config/envs.conf` 中 `FRESHQUANT_MONGODB__PORT` 改回 `27017`
- 重启三个宿主机进程
