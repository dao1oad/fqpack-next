...
| 0008 | TradingAgents-CN 集成（阶段 1：保持原生本地数据逻辑可用） | Done | Codex | 2026-03-07 | N/A（新增第三方能力） | `ta_backend` 已显式挂载 `third_party/tradingagents-cn/.env` 到容器 `/app/.env`，并在启动阶段通过 `bootstrap_reasoner_defaults` 自动补齐活动配置与 DeepSeek 模型目录；默认 A 股数据源已固定为 `Tushare > AKShare > BaoStock`，默认模型已切为 `quick_analysis_model=deepseek-chat`、`deep_analysis_model=deepseek-reasoner`。新增 `deepseek-reasoner` 的能力声明、provider 映射与仅针对工具调用兼容错误的精确回退逻辑；为避免 `your_*` 占位值误伤运行链路，配置桥接、分析服务和 memory 模块会统一忽略占位密钥。2026-03-07 用真实任务 `c4ffec0d-5878-4cdd-8c45-35cdc36db6e0` 完整验收 `002682`，任务完成并落库，`analysis_tasks.result.performance_metrics.llm_config.deep_think_model=deepseek-reasoner`，最终决策 `model_info=ChatDeepSeek:deepseek-reasoner`。同日已同步 RFC 0008 状态为 `Done`，补齐完成说明与验收勾选，收尾该迁移单元。 |
