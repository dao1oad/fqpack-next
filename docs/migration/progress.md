...
| 0013 | 融资账户仓位管理模块 | Implementing | Codex | 2026-03-07 | `D:\fqpack\freshquant\freshquant\strategy\toolkit\position_manager.py` / `position_risk_guard.py` / `order_manager.py` / xtquant `query_credit_detail` | 已在特性分支落地 `freshquant/position_management/`、独立分库常量与仓库层、独立 xtquant 信用资产查询 client、快照刷新 worker、三态策略与过旧保护、`OrderSubmitService` 策略门禁，以及 `FORCE_PROFIT_REDUCE` 下 Guardian 盈利卖点占位标志；待 review/merge 后再切为 Done。 |
