...
|---:|------|------|--------|----------|--------------------------|------|
| 0001 | Docker 并行部署（端口隔离） | Done | TBD | 2026-03-06 | N/A（部署形态） | `docker/compose.parallel.yaml` + 部署文档；覆盖 Web UI/API/TDXHQ/Dagster UI+daemon/Redis/Mongo/QAWebServer；修复 tdxhq `len(int)` 导致的 500；Web UI Nginx 增加 Docker DNS 动态解析避免 502；stock_data_job 排除 cjsd_data（避免数据同步触发 CJSD） |
