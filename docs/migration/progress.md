...
| 0013 | 融资账户仓位管理模块 | Approved | Codex | 2026-03-07 | `D:\fqpack\freshquant\freshquant\strategy\toolkit\position_manager.py` / `position_risk_guard.py` / `order_manager.py` / xtquant `query_credit_detail` | 设计与边界已确认：独立模块 `freshquant/position_management/`、独立分库 `freshquant_position_management`、独立 worker 周期刷新信用账户可用保证金、唯一状态源 `pm_current_state`、默认兜底状态 `HOLDING_ONLY`、仅控制 `source=strategy`。 |
