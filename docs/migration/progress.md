...
| 0008 | TradingAgents-CN 集成（阶段 1：保持原生本地数据逻辑可用） | Implementing | Codex | 2026-03-07 | N/A（新增第三方能力） | `ta_backend` 已改为显式挂载 `third_party/tradingagents-cn/.env` 到容器 `/app/.env`，并实测确认 Docker bind mount 生效；默认 A 股数据源已切为 `Tushare > AKShare > BaoStock`，默认分析模型已切为 `deepseek-chat`，`DeepSeek Chat` 工具调用能力元数据已补齐。为避免 `your_*` 占位值误伤运行链路，已在配置桥接、分析服务和 memory 模块统一忽略占位密钥；当仅有 `DeepSeek` 聊天凭证而无有效 embedding provider 时，memory 会安静降级为禁用，不再触发 `DashScope InvalidApiKey` 或无效 `/embeddings` 请求。2026-03-06 23:55 发起真实任务 `7740f2f7-daaa-444e-a369-6baef00f3029`，已于 2026-03-07 00:11 完整跑通并返回结果 `analysis_id=298df2a5-51ba-48c4-aab2-f2e23e5b716d`。 |
