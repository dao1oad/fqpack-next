...
import pandas as pd
from backtrader.analyzers import SQN, SharpeRatio, TimeReturn
                ...
                if position is None or position.size == 0:
                    ...
                    for s in range(len(bi), 0, -1):
                        if bi[s - 1] == -1:
                            ...
                            self.win_price[i] = self.buy_price[i] + self.params.ror * (
                                self.buy_price[i] - self.stop_price[i]
                            )
                            break
                    ...
                    t.update(
                        {
                            ...
                            'dtopen': bt.num2date(trade.dtopen),
                            'dtclose': (
                                bt.num2date(trade.dtclose) if trade.isclosed else None
                            ),
                        }
                    )
                    break
            print(
                f"{bt.num2date(trade.dtclose)} {trade.data._name} 策略收益：毛收益 {trade.pnl:.2f}, 净收益 {trade.pnlcomm:.2f}"
            )
