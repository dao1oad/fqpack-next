...
# ------------------  1级：应用层函数(通过0级核心函数实现）（补充）--------------------------------------------
# 上穿函数
def CROSS_UP(S1, S2):
    CROSS_BOOL = IF(S1 > S2, True, False)
    tmp1 = COUNT(CROSS_BOOL > 0, 2) == 1
    tmp2 = COUNT(CROSS_BOOL > 0, 1) == 1
    rotary = np.empty(len(S1), dtype=np.int32)
    for i in range(0, len(CROSS_BOOL)):
        if (tmp1[i] == True) and (tmp2[i] == True):
            rotary[i] = 1
        else:
            rotary[i] = 0
    return rotary
