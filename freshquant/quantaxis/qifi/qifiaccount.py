...
class QIFI_Account:
    ...
    def initial(self):
        if not self.nodatabase:
            self.db = pymongo.MongoClient(self.trade_host).freshquant
            if self.model.upper() == "BACKTEST":
                self.account_col = self.db.backtest_account
                self.account_his_col = self.db.backtest_account_his
            elif self.model.upper() == "SIM":
                self.account_col = self.db.sim_account
                self.account_his_col = self.db.sim_account_his
            elif self.model.upper() == "REAL":
                self.account_col = self.db.real_account
                self.account_his_col = self.db.real_account_his
            else:
                raise RuntimeError("model should be in BACKTEST/ SIM/ REAL")
            self.reload()
        else:
            """
            非数据库模式  不用 reload
    ...
    def reload(self):
        message = self.account_col.find_one({'account_cookie': self.user_id})
        ...
        if message is not None:
            ...
            for position in positions.values():
                self.positions[
                    position.get('exchange_id') + '.' + position.get('instrument_id')
    ...
    def sync(self):
        if self.trading_day != '':
            self.log(f"sync for {self.trading_day}")
            self.on_sync()
            try:
                if not self.nodatabase:
                    self.account_col.update_one(
                        {'account_cookie': self.user_id},
                        {'$set': self.message},
                        upsert=True,
                    )
                    self.account_his_col.update_one(
                        {
                            'account_cookie': self.user_id,
    ...
    def order_check(
        self, code: str, amount: int, price: float, towards: int, order_id: str
    ) -> bool:
        """
    ...
    def send_order(
        ...
        if self.order_check(code, int(amount), price, towards, order_id):
    ...
    def receive_deal(
        ...
        if order_id in self.orders.keys():
            ...
            if pos.volume_long == 0 and pos.volume_short == 0:
                self.positions.pop(self.format_code(code), None)
            if self.model != "BACKTEST":
                self.sync()

    def get_position(self, code: Optional[str] = None) -> QA_Position:
        """
        兼容 code.XSHE 诸如
