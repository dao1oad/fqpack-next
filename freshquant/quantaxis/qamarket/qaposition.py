...
from QUANTAXIS.QAUtil.QAParameter import MARKET_TYPE, ORDER_DIRECTION, ORDER_STATUS
...
class QA_Position:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.moneypresetLeft = (
            self.moneypreset if moneypresetLeft is None else moneypresetLeft
        )
        '''{'name': '原油',
            'unit_table': 1000,
            'price_tick': 0.1,
        ...
        self.exchange_id = (
            self.market_preset['exchange'] if exchange_id == None else exchange_id
        )
        ...
        self.position_price_long = (
            open_price_long if position_price_long == 0 else position_price_long
        )
        self.position_price_short = (
            open_price_short if position_price_short == 0 else position_price_short
        )

        self.open_cost_long = (
            open_cost_long
            if open_cost_long != 0
            else open_price_long
            * self.volume_long
            * self.market_preset.get('unit_table', 1)
        )
        self.open_cost_short = (
            open_cost_short
            if open_cost_short != 0
            else open_price_short
            * self.volume_short
            * self.market_preset.get('unit_table', 1)
        )
        self.position_cost_long = (
            position_cost_long
            if position_cost_long != 0
            else self.position_price_long
            * self.volume_long
            * self.market_preset.get('unit_table', 1)
        )
        self.position_cost_short = (
            position_cost_short
            if position_cost_short != 0
            else self.position_price_short
            * self.volume_short
            * self.market_preset.get('unit_table', 1)
        )
        ...
        self.oms_id = oms_id
        # 存储额外的关键字参数
        self.extra = kwargs.copy() if kwargs else {}
    ...
    def __repr__(self):
        return '< QAPOSITION {} amount {}/{} >'.format(
            self.code, self.volume_long, self.volume_short
        )
    ...
    def order_check(
        self, amount: int, price: float, towards: int, order_id: str
    ) -> bool:
        res = False
        if towards == ORDER_DIRECTION.BUY_CLOSE:
            ...
            moneyneed = (
                ...
                * float(self.market_preset.get('buy_frozen_coeff', 1))
            )
    ...
    def update_pos(self, price, amount, towards):
        ...
        temp_cost = (
            int(amount) * float(price) * float(self.market_preset.get('unit_table', 1))
        )
        profit = 0
        if towards == ORDER_DIRECTION.BUY:
            ...
            if self.volume_long_his >= amount:
                ...
                if self.volume_long > 0:
                    self.position_cost_long = (
                        self.position_cost_long * (self.volume_long) / self.volume_long
                        + amount
                    )
                    self.open_cost_long = (
                        self.open_cost_long
            ...
            # 增加保证金
            marginValue = temp_cost * self.market_preset.get('buy_frozen_coeff', 1)
            self.margin_long += marginValue
            # 重算开仓均价
            self.open_price_long = (
                self.open_price_long * self.volume_long + amount * price
            ...
            6. 增加空单仓位
            '''
            marginValue = temp_cost * self.market_preset.get('sell_frozen_coeff', 1)
            ...
            if self.volume_short_today > amount:
                ...
                self.margin_short += marginValue
                profit = (
                    (self.position_price_short - price)
                    * amount
                    * self.market_preset.get('unit_table', 1)
                )
            ...
            if self.volume_long_today > amount:
                ...
                self.open_cost_long = (
                    self.open_cost_long * self.volume_long / (self.volume_long + amount)
                )
                ...
                self.margin_long += marginValue
                profit = (
                    (price - self.position_price_long)
                    * amount
                    * self.market_preset.get('unit_table', 1)
                )
                self.moneypresetLeft += -marginValue + profit
    ...
    def calc_commission(
        ...
        if self.market_type == MARKET_TYPE.FUTURE_CN:
            # 期货不收税
            # 双边手续费 也没有最小手续费限制
            value = trade_price * trade_amount * MARKET_PRESET().get_unit(self.code)
            ...
            if trade_towards in [
                ...
                commission_fee = commission_fee_preset[
                    'commission_coeff_pervol'
                ] * trade_amount + commission_fee_preset[
    ...
    def loadfrommessage(self, message):
        ...
        if self.volume_long + self.volume_short > 0:
            self.last_price = (
                self.open_price_long * self.volume_long
            ...
            order = self.send_order(
                ...
                eval(
                    'ORDER_DIRECTION.{}_{}'.format(
                        order.get('direction'), order.get('offset')
                    )
                ),
            )
    ...
    def on_transaction(self, transaction: dict):
        towards = transaction.get(
            ...
            eval(
                'ORDER_DIRECTION.{}_{}'.format(
                    transaction.get('direction'), transaction.get('offset')
                )
            ),
        )
        ...
        try:
            self.update_pos(
                transaction['price'],
                transaction.get('amount', transaction.get('volume')),
                towards,
            )
