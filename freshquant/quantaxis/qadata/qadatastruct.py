...
class QA_DataStruct_Stock_day(_quotation_base):
    ...
    def to_hfq(self):
        if self.if_fq == 'bfq':
            if len(self.code) < 1:
                ...
                return self.new(
                    self.groupby(level=1).apply(QA_data_stock_to_fq, 'hfq'),
                    self.type,
                    'hfq',
                )
    ...
    def high_limit(self):
        '涨停价'
        return (
            self.groupby(level=1)
            .close.apply(lambda x: round((x.shift(1) + 0.0002) * 1.1, 2))
            .sort_index()
        )
    ...
    def low_limit(self):
        ...
        return (
            self.groupby(level=1)
            .close.apply(lambda x: round((x.shift(1) + 0.0002) * 0.9, 2))
            .sort_index()
        )
    ...
    def next_day_low_limit(self):
        ...
        return (
            self.groupby(level=1)
            .close.apply(lambda x: round((x + 0.0002) * 0.9, 2))
            .sort_index()
        )
    ...
    def next_day_high_limit(self):
        ...
        return (
            self.groupby(level=1)
            .close.apply(lambda x: round((x + 0.0002) * 1.1, 2))
            .sort_index()
        )
...
class QA_DataStruct_Stock_transaction:
    ...
    def get_medium_order(self, lower=200000, higher=1000000):
        ...
        return self.data.query('amount>={}'.format(lower)).query(
            'amount<={}'.format(higher)
        )
...
class QA_DataStruct_Stock_realtime(_realtime_base):
    ...
    def to_json(self):
        return self.data.assign(code=self.code, datetime=str(self.datetime)).to_dict(
            orient='records'
        )
...
class QA_DataStruct_Stock_realtime_series:
    def __init__(self, sr_series):
        ...
        if isinstance(sr_series[0], QA_DataStruct_Stock_realtime):
            ...
            self.sr_series = [QA_DataStruct_Stock_realtime(sr) for sr in sr_series]
...
class QA_DataStruct_CryptoCurrency_min(_quotation_base):
    ...
    def resample(self, level):
        try:
            return self.add_funcx(
                QA_data_cryptocurrency_min_resample, level
            ).sort_index()
        except Exception as e:
            print('QA ERROR : FAIL TO RESAMPLE {}'.format(e))
            return None
