...
def save_hq_stock_realtime(records: list, collection: str = "stock_realtime"):
    ...
    try:
        ...
        last_records = list(
            DBfreshquant[collection]
            .find({"code": code, "frequence": frequence}, sort=[("datetime", -1)])
            .limit(10)
        )
        if last_records and len(last_records) > 0:
            ...
            # 保留所有新数据和重叠的数据
            records = [
                record for record in records if record["datetime"] >= cutoff_datetime
            ]
        if len(records) == 0:
            return
...
def process_hq_stock_realtime(df: pd.DataFrame, collection: str = "stock_realtime"):
    ...
    # 合成5分钟的数据
    df = df.resample('5T', closed='right', label='right').agg(OHLC).dropna(how='any')
    df["code"] = code
    df["frequence"] = "5min"
    df["source"] = source
    df['time_stamp'] = df.index.to_series().apply(lambda v: QA_util_time_stamp(v))
    ...
    if len(df) > 1:
        save_hq_stock_realtime(
            df[1:].reset_index().to_dict(orient="records"), collection
        )

    # 合成15分钟的数据
    df = df.resample('15T', closed='right', label='right').agg(OHLC).dropna(how='any')
    df["code"] = code
    df["frequence"] = "15min"
    df["source"] = source
    df['time_stamp'] = df.index.to_series().apply(lambda v: QA_util_time_stamp(v))
    ...
    # 合成30分钟的数据
    df = df.resample('30T', closed='right', label='right').agg(OHLC).dropna(how='any')
    df["code"] = code
    df["frequence"] = "30min"
    df["source"] = source
    df['time_stamp'] = df.index.to_series().apply(lambda v: QA_util_time_stamp(v))
    ...
    # 先合成日线数据再过滤
    df_daily = (
        df.resample('1D', closed='left', label='left').agg(OHLC).dropna(how='any')
    )
    df_daily["code"] = code
    df_daily["frequence"] = "1d"
    df_daily["source"] = source
    df_daily['time_stamp'] = df_daily.index.to_series().apply(
        lambda v: QA_util_time_stamp(v)
    )
    ...
    if len(df_daily) > 0:
        # 保持与其他周期一致，去掉第一条数据
        save_hq_stock_realtime(
            df_daily.reset_index().to_dict(orient="records"), collection
        )
