...
class CoalescingScheduler:
    ...
    def __init__(self, *, max_inflight: int, submit_fn: Callable[[Key, Any], Any]):
        self.max_inflight = max(1, int(max_inflight))
        self._submit_fn = submit_fn
        self._states: dict[Key, CoalesceState] = {}
        self._inflight_total = 0

    def update(self, key: Key, latest: Any) -> None:
    ...
    def mark_done(self, key: Key) -> None:
        st = self._states.get(key)
    ...
    def snapshot(self) -> dict[str, int]:
        ...
        for st in self._states.values():
            ...
            if st.inflight:
                inflight += 1
        return {"keys": len(self._states), "pending": pending, "inflight": inflight, "inflight_total": self._inflight_total}
