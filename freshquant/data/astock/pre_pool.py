...
def remove(id=None, category=None, codes=None):
    ...
    if id:
        ...
        # 删除整个category
        DBfreshquant["stock_pre_pools"].delete_many({"category": category})
    elif codes:
        # 从所有category中删除指定codes
        DBfreshquant["stock_pre_pools"].delete_many({"code": {"$in": codes}})
    else:
...
def save_a_stock_pre_pool(
    ...
    **extra_fields,
    ...
    if not append_mode:
        DBfreshquant.stock_pre_pools.delete_many(
            {"category": category, "code": {"$nin": codes}}
        )
    ...
    for code in codes:
        ...
        if instrument is not None:
            ...
            existing_doc = DBfreshquant.stock_pre_pools.find_one(
            ...
            DBfreshquant.stock_pre_pools.find_one_and_update(
                {"code": code, "category": category},
                {
                    "$set": {"expire_at": expire_at, "extra": extra},  # 更新 extra 字段
                    "$setOnInsert": {
                        "datetime": dt_now,
...
def query_a_stock_pre_pool(categories: List[str] = [], **query_params):
