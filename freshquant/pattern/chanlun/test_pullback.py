...
async def fetch_stock_day_data(
    code, dt: datetime = datetime.now()
    ...
    try:
        ...
        logger.info(
            f"Error fetching data for stock {code}: {e} {traceback.format_exc()}"
        )
        return None
...
async def process_stock_pullback(stock, days: int = 1) -> Optional[list]:
    ...
    if stock_day_data is not None and len(stock_day_data) > 0:
        ...
        try:
                    ...
                    result.append(
                        {
                            ...
                            "stop_lose_rate": f'{round(((buy_signals["price"][i] - buy_signals["stop_lose_price"][i]) / buy_signals["price"][i]) * 100, 2)}%',
                            "amount": (
                                amounts[buy_signals['idx'][i]]
                                if buy_signals['idx'][i] < len(amounts)
                                else None
                            ),
                        }
                    )
                    ...
                    result.append(
                        {
                            ...
                            "stop_lose_rate": f'{round(((sell_signals["stop_lose_price"][i] - sell_signals["price"][i]) / sell_signals["price"][i]) * 100, 2)}%',
                            "amount": (
                                amounts[sell_signals['idx'][i]]
                                if sell_signals['idx'][i] < len(amounts)
                                else None
                            ),
                        }
                    )
            ...
            logger.error(
                f"Error processing pullback for stock {stock.get('code')}: {e}"
            )

    return result


async def test_pullback(
    days: int = 1, code: Optional[str] = None, limit: Optional[int] = None
):
    ...
    if len(valid_records) > 0:
        ...
        print(
            f"买入拉回信号: {len(result_df[result_df['signal_type'] == 'buy_zs_huila'])}"
        )
        print(
            f"卖出拉回信号: {len(result_df[result_df['signal_type'] == 'sell_zs_huila'])}"
        )
        ...
        display_df = (
            ...
            if all(col in result_df.columns for col in display_columns)
            else result_df
        )
        ...
        styled_result_df = (
            result_df.style.set_caption(f"{days_text}内拉回信号结果")
            .set_table_styles(
                [
                    {
                        'selector': 'caption',
                    ...
                    {
                        'selector': 'td',
                        'props': [('text-align', 'left'), ('padding', '4px')],
                    },
                ]
            )
            ...
            .set_properties(
                subset=['signal_type'], **{'color': 'darkgreen', 'font-weight': 'bold'}
            )
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--days", type=int, default=1, help="Number of days to consider for testing"
    )
    parser.add_argument(
        "--code", type=str, help="Specific stock code to test (optional)"
    )
    parser.add_argument(
        "--limit", type=int, help="Limit number of stocks to test (optional)"
    )

    args = parser.parse_args()

    asyncio.run(test_pullback(days=args.days, code=args.code, limit=args.limit))
