...
async def process_stock_macd_divergence(stock, days: int = 1) -> Optional[list]:
    ...
    if stock_day_data is not None and len(stock_day_data) > 0:
        ...
        try:
                    ...
                    result.append(
                        {
                            ...
                            "stop_lose_rate": f'{round(((bullish_signals["price"][i] - bullish_signals["stop_lose_price"][i]) / bullish_signals["price"][i]) * 100, 2)}%',
                            "zhongshu_count": bullish_signals['zhongshu_count'][i],
                            "amount": (
                                amounts[bullish_signals['idx'][i]]
                                if bullish_signals['idx'][i] < len(amounts)
                                else None
                            ),
                        }
                    )
                    ...
                    result.append(
                        {
                            ...
                            "stop_lose_rate": f'{round(((bearish_signals["stop_lose_price"][i] - bearish_signals["price"][i]) / bearish_signals["price"][i]) * 100, 2)}%',
                            "zhongshu_count": bearish_signals['zhongshu_count'][i],
                            "amount": (
                                amounts[bearish_signals['idx'][i]]
                                if bearish_signals['idx'][i] < len(amounts)
                                else None
                            ),
                        }
                    )
            ...
            logger.error(
                f"Error processing MACD divergence for stock {stock.get('code')}: {e}"
            )

    return result


async def test_macd_divergence(
    ...
    if len(valid_records) > 0:
        ...
        styled_result_df = (
            result_df.style.set_caption(f"{days_text}内MACD背驰信号结果")
            .set_table_styles(
                [
                    {
                        'selector': 'caption',
...
if __name__ == "__main__":
    ...
    asyncio.run(test_macd_divergence(days=args.days, code=args.code, limit=args.limit))
