...
def _capture_fqcopilot_setup(
    monkeypatch: pytest.MonkeyPatch, *, target: str | None = None
) -> dict[str, object]:
    ...
    if target is None:
        monkeypatch.delenv("FQCOPILOT_BUILD_TARGET", raising=False)
    else:
        monkeypatch.setenv("FQCOPILOT_BUILD_TARGET", target)
...
def test_fqcopilot_setup_declares_fullcalc_extension(monkeypatch) -> None:
    captured = _capture_fqcopilot_setup(monkeypatch)

    ext_modules = cast(list[Any] | None, captured.get("ext_modules"))
    assert ext_modules is not None
    ext_names = {ext.name for ext in ext_modules}
    assert ext_names == {"fqcopilot", "fullcalc"}
    fullcalc_ext = next(ext for ext in ext_modules if ext.name == "fullcalc")
    assert any(
        source.replace("\\", "/").endswith("../cpp/chanlun/czsc.cpp")
        for source in fullcalc_ext.sources
    )
    assert not any("fqchan04" in source for source in fullcalc_ext.sources)
    assert any(
...
@pytest.mark.parametrize(
    ...
    [
        ("fqcopilot", {"fqcopilot"}),
        ("fullcalc", {"fullcalc"}),
        ("all", {"fqcopilot", "fullcalc"}),
    ],
)
def test_fqcopilot_setup_selects_an_explicit_build_target(
    monkeypatch: pytest.MonkeyPatch,
    target: str,
    expected_extensions: set[str],
) -> None:
    captured = _capture_fqcopilot_setup(monkeypatch, target=target)

    ext_modules = cast(list[Any], captured["ext_modules"])
    assert {ext.name for ext in ext_modules} == expected_extensions


def test_fqcopilot_setup_rejects_an_unknown_build_target(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    with pytest.raises(ValueError, match="FQCOPILOT_BUILD_TARGET"):
        _capture_fqcopilot_setup(monkeypatch, target="unknown")
