    ...
    for (int i = 0; i < nCount; i++)
    {
        if (pIn[i] == 1)
        {
            ...
            if (ZhongShuOne.PushHigh(i, pHigh[i]))
            {
                ...
                {
                    Pivot pivot;
                    pivot.start = ZhongShuOne.nStart;
                    pivot.end = ZhongShuOne.nEnd;
                    pivot.zg = ZhongShuOne.fHigh;
                    pivot.zd = ZhongShuOne.fLow;
                    pivot.direction = (float)ZhongShuOne.nDirection;
                    pivot.gg = *std::max_element(pHigh.begin() + pivot.start + 1, pHigh.begin() + pivot.end, [](float a, float b)
                                                 { return a < b; });
                    pivot.dd = *std::min_element(pLow.begin() + pivot.start + 1, pLow.begin() + pivot.end, [](float a, float b)
                                                 { return a < b; });
