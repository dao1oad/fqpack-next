<template>
  <el-drawer :model-value="show" size="min(960px, 94vw)" direction="rtl" @update:model-value="$emit('update:show', $event)">
    ...
    <div v-if="signal" class="clx-kline__facts"><div><span>信号日</span><b>{{ signal.signalDate }}</b></div><div><span>可见日</span><b>{{ signal.revealDate }}</b></div><div v-if="signal.decisionRevealDate"><span>组合决策日</span><b>{{ signal.decisionRevealDate }}</b></div><div v-if="signal.decisionDirection"><span>决策方向</span><b>{{ signal.decisionDirection }}</b></div><div><span>第几次</span><b>{{ signal.occurrence || '--' }}</b></div><div><span>主触发</span><b>{{ signal.primaryTrigger || '--' }}</b></div><div class="clx-kline__wide"><span>同K线并发触发</span><b>{{ signal.concurrentTriggers.join(' · ') || '--' }}</b></div></div>
