...
test('PositionManagement.vue uses merged left panel and fully visible rule matrix without a separate inventory card', async () => {
  ...
  assert.ok(actionHeaderIndex < brokerHeaderIndex)
  assert.ok(brokerHeaderIndex < inferredHeaderIndex)
  assert.ok(inferredHeaderIndex < stockFillsHeaderIndex)
  assert.match(content, /position-symbol-limit-actions[\s\S]*saveSymbolLimit\(row\)[\s\S]*row\.broker_position_source_label[\s\S]*row\.inferred_position_source_label[\s\S]*row\.legacy_position_source_label/)
...
test('PositionManagement.vue places rule matrix above inventory, shrinks rule card into the shared card grid, and fixes position columns after actions', async () => {
  ...
  assert.match(content, /--position-symbol-limit-position-column-width:\s*188px;/)
  assert.match(content, /\.position-lower-grid\s*\{[\s\S]*align-items:\s*stretch;/)
  assert.match(content, /runtime-position-symbol-limit-ledger__grid\s*\{[\s\S]*144px[\s\S]*92px[\s\S]*144px[\s\S]*88px[\s\S]*92px[\s\S]*72px[\s\S]*84px[\s\S]*var\(--position-symbol-limit-position-column-width\)\s+var\(--position-symbol-limit-position-column-width\)\s+var\(--position-symbol-limit-position-column-width\)/)
  assert.doesNotMatch(content, /position-symbol-limit-source-column-min-width/)
  assert.doesNotMatch(content, /minmax\(var\(--position-symbol-limit-source-column-min-width\),\s*1\.1fr\)/)
...
test('position-management module doc reflects merged left panel, dirty-symbol filtering and truth-filled recent decisions', async () => {
  ...
  assert.match(content, /券商仓位 \/ 订单推断仓位 \/ stock_fills 仓位.*挪到“操作”列后/)
  assert.match(content, /券商仓位 \/ 订单推断仓位 \/ stock_fills 仓位.*固定宽度列/)
