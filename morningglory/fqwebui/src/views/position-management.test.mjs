...
test('PositionManagement.vue renders the final dense two-column workbench with state panel above subject overview', async () => {
  ...
  assert.match(content, /<WorkbenchDetailPanel class="position-state-panel">[\s\S]*<PositionSubjectOverviewPanel class="position-subject-overview-host"/)
...
test('PositionManagement.vue hoists subject workbench state while keeping recent decisions global', async () => {
  ...
  assert.match(content, /--position-workbench-right-width:\s*0\.94/)
  assert.match(content, /--position-workbench-left-width:\s*1\.18/)
  assert.match(content, /label="入口" width="96"/)
  assert.match(content, /label="入口" width="108"/)
  assert.match(content, /label="买入时间" width="148"/)
  assert.match(content, /label="剩余 \/ 占比" width="148"/)
  ...
  assert.match(subjectOverviewPanelSource, /<el-table-column label="标的" width="84" fixed="left">/)
  assert.match(subjectOverviewPanelSource, /<el-table-column label="Guardian 买入层级" width="172">/)
  assert.match(subjectOverviewPanelSource, /<el-table-column label="止盈价格层级" width="172">/)
  assert.match(subjectOverviewPanelSource, /<el-table-column label="Guardian 层级触发" width="220">/)
  assert.match(subjectOverviewPanelSource, /<el-table-column label="止盈层级触发" width="220">/)
  assert.match(subjectOverviewPanelSource, /<el-table-column label="单笔止损触发" width="220">/)
...
test('position-management module doc reflects independent reconciliation panel and read-only audit semantics', async () => {
  ...
  assert.match(content, /对账中心/)
  assert.match(content, /`\/reconciliation`/)
