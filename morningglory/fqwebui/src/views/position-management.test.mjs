...
test('PositionManagement.vue hoists subject workbench state and filters right-column detail by the selected symbol', async () => {
  const content = await readFile(new URL('./PositionManagement.vue', import.meta.url), 'utf8')
  ...
  assert.match(content, /filteredDecisionLedgerRows/)
  ...
  assert.match(subjectOverviewPanelSource, /label="分类"/)
  assert.match(subjectOverviewPanelSource, /label="持仓股数"/)
  assert.match(subjectOverviewPanelSource, /label="持仓市值"/)
  assert.match(subjectOverviewPanelSource, /label="止损价"/)
  assert.match(subjectOverviewPanelSource, /label="首笔金额"/)
  assert.match(subjectOverviewPanelSource, /label="常规金额"/)
  assert.match(subjectOverviewPanelSource, /label="单标的上限"/)
  assert.match(subjectOverviewPanelSource, /label="活跃止损"/)
...
test('position-management module doc reflects independent reconciliation panel and read-only audit semantics', async () => {
  ...
  assert.match(content, /选中标的工作区当前展示：聚合买入列表 \/ 按持仓入口止损 与 切片明细/)
