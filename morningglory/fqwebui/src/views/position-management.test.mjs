...
test('PositionManagement.vue places rule matrix above inventory, keeps the top grid aligned, and separates reconciliation into its own panel', async () => {
  ...
  assert.match(content, /runtime-position-symbol-limit-ledger__grid\s*\{[\s\S]*144px[\s\S]*144px[\s\S]*84px[\s\S]*88px[\s\S]*72px/)
  assert.doesNotMatch(content, /runtime-position-symbol-limit-ledger__grid\s*\{[\s\S]*92px[\s\S]*var\(--position-symbol-limit-position-column-width\)/)
  assert.match(content, /position-reconciliation-panel/)
