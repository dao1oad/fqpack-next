...
test('PositionManagement.vue hoists subject workbench state while keeping recent decisions global', async () => {
  ...
  const guardianLevelColumnIndex = subjectOverviewPanelSource.indexOf('label="Guardian 买入层级（配置）"')
  ...
  assert.match(subjectOverviewPanelSource, /label="Guardian 买入层级（配置）"/)
  ...
  assert.match(subjectOverviewPanelSource, /<el-table-column label="Guardian 买入层级（配置）" min-width="128">/)
...
test('position-management module doc reflects independent reconciliation panel and read-only audit semantics', async () => {
  ...
  assert.match(content, /Guardian 买入层级（配置）/)
