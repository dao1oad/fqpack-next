...
test('PositionManagement.vue hoists subject workbench state while keeping recent decisions global', async () => {
  ...
  const gateColumnIndex = subjectOverviewPanelSource.indexOf('label="门禁"')
  const guardianTriggerColumnIndex = subjectOverviewPanelSource.indexOf('label="Guardian 层级触发"')
  const tpslTriggerColumnIndex = subjectOverviewPanelSource.indexOf('label="TPLS触发"')
  ...
  assert.match(subjectOverviewPanelSource, /label="门禁"/)
  assert.match(subjectOverviewPanelSource, /label="Guardian 层级触发"/)
  assert.match(subjectOverviewPanelSource, /label="TPLS触发"/)
  ...
  assert.match(subjectOverviewPanelSource, /<el-table-column label="标的" width="88" fixed="left">/)
  assert.match(subjectOverviewPanelSource, /<el-table-column label="门禁" width="128">/)
  assert.match(subjectOverviewPanelSource, /<el-table-column label="Guardian 层级触发" width="170">/)
  assert.match(subjectOverviewPanelSource, /<el-table-column label="TPLS触发" width="170">/)
  assert.match(subjectOverviewPanelSource, /<el-table-column label="Guardian 买入层级" width="164">/)
  assert.match(subjectOverviewPanelSource, /<el-table-column label="止盈价格层级" width="164">/)
  ...
  assert.ok(gateColumnIndex > orderStatusColumnIndex)
  assert.ok(guardianTriggerColumnIndex > gateColumnIndex)
  assert.ok(tpslTriggerColumnIndex > guardianTriggerColumnIndex)
  assert.ok(guardianLevelColumnIndex > tpslTriggerColumnIndex)
  ...
  assert.match(subjectOverviewPanelSource, /row\.tpslTrigger\?\.kindLabel/)
  assert.match(subjectOverviewPanelSource, /position-subject-gate-chip/)
