...
test('PositionManagement.vue renders the final three-column workbench with state and audit on the left, subject overview in the middle, and recent decisions on the right', async () => {
...
test('PositionManagement.vue reuses subject-management workbench logic for inline config and stoploss editing inside the middle table', async () => {
  const content = await readFile(new URL('./PositionManagement.vue', import.meta.url), 'utf8')
  ...
  assert.match(content, /PositionSubjectOverviewPanel/)
  assert.match(subjectOverviewPanelSource, /subjectManagementApi/)
  assert.match(subjectOverviewPanelSource, /createPositionManagementSubjectWorkbenchController/)
  assert.match(subjectOverviewPanelSource, /buildDenseConfigRows/)
  assert.match(subjectOverviewPanelSource, /detailMap/)
  assert.match(subjectOverviewPanelSource, /mustPoolDrafts/)
  assert.match(subjectOverviewPanelSource, /positionLimitDrafts/)
  assert.match(subjectOverviewPanelSource, /stoplossDrafts/)
  assert.match(subjectOverviewPanelSource, /ensureSubjectDetailsForPage/)
  assert.match(subjectOverviewPanelSource, /saveConfigBundleForSymbol/)
  assert.match(subjectOverviewPanelSource, /saveStoplossForEntry/)
  assert.match(subjectOverviewPanelSource, /searchSubjectKeyword/)
  assert.match(subjectOverviewPanelSource, /selectedSubjectCategory/)
  assert.match(subjectOverviewPanelSource, /基础配置 \+ 单标的仓位上限/)
  assert.match(subjectOverviewPanelSource, /聚合买入列表/)
  assert.match(subjectOverviewPanelSource, /切片明细/)
  assert.match(subjectOverviewPanelSource, /el-popover/)
  assert.match(subjectOverviewPanelSource, /aggregation_members/)
  assert.match(subjectOverviewPanelSource, /entry_slices/)
  assert.match(subjectOverviewPanelSource, /buildDenseConfigRows\(detailMap\[row\.symbol\]/)
  assert.match(subjectOverviewPanelSource, /handleSubjectOverviewPageChange/)
  assert.match(subjectOverviewPanelSource, /handleSubjectOverviewPageSizeChange/)
  assert.doesNotMatch(subjectOverviewPanelSource, /subject-editor-stack/)
  assert.doesNotMatch(subjectOverviewPanelSource, /左侧先选择一个标的/)
})
...
test('position-management module doc reflects independent reconciliation panel and read-only audit semantics', async () => {
  ...
  assert.match(content, /右栏：最近决策与上下文/)
  ...
  assert.match(content, /卡片列表当前展示：/)
  assert.match(content, /展开证据当前展示：/)
