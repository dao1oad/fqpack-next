...
test('PositionManagement.vue renders the final dense three-column workbench with split left and right columns', async () => {
  ...
  assert.match(content, /label="剩余数量"/)
  assert.match(content, /label="百分比"/)
  assert.match(content, /label="剩余市值"/)
...
test('PositionManagement.vue hoists subject workbench state while keeping recent decisions global', async () => {
  ...
  const positionQuantityColumnIndex = subjectOverviewPanelSource.indexOf('label="持仓股数"')
  const activeStoplossColumnIndex = subjectOverviewPanelSource.indexOf('label="活跃单笔止损"')
  const openEntryColumnIndex = subjectOverviewPanelSource.indexOf('label="Open Entry"')
  const latestTriggerColumnIndex = subjectOverviewPanelSource.indexOf('label="最近TPLS触发"')
  const positionAmountColumnIndex = subjectOverviewPanelSource.indexOf('label="持仓市值"')
  const guardianLevelColumnIndex = subjectOverviewPanelSource.indexOf('label="Guardian 层级买入"')
  const guardianTriggerColumnIndex = subjectOverviewPanelSource.indexOf('label="Guardian层级触发"')
  const takeprofitColumnIndex = subjectOverviewPanelSource.indexOf('label="止盈价格"')
  ...
  assert.match(content, /row\.entrySummaryDisplay\?\.remainingQuantityLabel/)
  assert.match(content, /row\.entrySummaryDisplay\?\.remainingPercentLabel/)
  ...
  assert.match(content, /width="248"/)
  ...
  assert.match(subjectOverviewPanelSource, /label="Guardian层级触发"/)
  ...
  assert.match(subjectOverviewPanelSource, /最近TPLS触发/)
  assert.match(subjectOverviewPanelSource, /<el-table-column label="标的" width="100" fixed="left">/)
  assert.ok(symbolColumnIndex >= 0)
  assert.ok(positionQuantityColumnIndex >= 0)
  assert.ok(activeStoplossColumnIndex > positionQuantityColumnIndex)
  assert.ok(openEntryColumnIndex > activeStoplossColumnIndex)
  assert.ok(latestTriggerColumnIndex > openEntryColumnIndex)
  assert.ok(positionAmountColumnIndex > latestTriggerColumnIndex)
  assert.ok(guardianLevelColumnIndex > positionAmountColumnIndex)
  assert.ok(guardianTriggerColumnIndex > guardianLevelColumnIndex)
  assert.ok(takeprofitColumnIndex > guardianTriggerColumnIndex)
...
test('position-management module doc reflects independent reconciliation panel and read-only audit semantics', async () => {
  ...
  assert.match(content, /剩余数量/)
  assert.match(content, /百分比/)
  assert.match(content, /剩余市值/)
  assert.match(content, /单笔止损/)
  assert.match(content, /最近TPLS触发/)
  assert.match(content, /takeprofit \/ stoploss/)
