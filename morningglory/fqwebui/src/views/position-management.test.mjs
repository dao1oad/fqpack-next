...
test('PositionManagement.vue renders the final dense two-column workbench with state panel above subject overview', async () => {
  ...
  assert.match(content, /label="差异处理"/)
  assert.match(content, /class="position-selection-grid position-selection-grid--orders"/)
  assert.match(content, /class="position-selection-grid position-selection-grid--columns"/)
  assert.match(content, /聚合买入列表/)
  assert.match(content, /切片明细（\{\{\s*selectedSubjectSliceRows\.length\s*\}\}）/)
  assert.match(content, /订单列表/)
  assert.match(content, /订单详情/)
  assert.match(content, /label="成交回报"/)
  assert.match(content, /label="状态流转"/)
  assert.match(content, /label="基础信息"/)
  assert.match(content, /规则检查/)
  assert.match(content, /视图对照/)
  assert.match(content, /差异处理列表/)
  ...
  assert.match(content, /<el-table-column label="更新时间" min-width="136" show-overflow-tooltip>/)
  assert.match(content, /<el-table-column label="状态" width="112" show-overflow-tooltip>/)
  assert.match(content, /<el-table-column prop="label" label="说明" min-width="112" show-overflow-tooltip \/>/)
  assert.match(content, /<el-table-column prop="expected_relation" label="关系" min-width="108" show-overflow-tooltip \/>/)
  assert.match(content, /<el-table-column label="结果" width="112">/)
  assert.match(content, /<el-table-column label="id" width="84" show-overflow-tooltip>/)
  assert.match(content, /<el-table-column label="类型" width="112">/)
  assert.match(content, /class="position-selection-detail-body"/)
  assert.match(content, /class="position-order-detail-tabs"/)
  assert.match(content, /const orderDetailTab = ref\('trades'\)/)
  assert.match(content, /orderDetailTab\.value = 'trades'/)
  assert.match(content, /当前订单暂无状态流转记录/)
  assert.match(content, /当前订单暂无成交回报/)
  assert.match(content, /const compactTailId = \(value, length = 6\) => \{/)
  assert.match(content, /compactTailId\(row\.row_id\)/)
  assert.match(content, /<el-table-column prop="state" label="state" width="112" show-overflow-tooltip \/>/)
  ...
  assert.doesNotMatch(content, /class="workbench-block position-troubleshoot-block/)
  assert.doesNotMatch(content, /label="Resolution"/)
  assert.doesNotMatch(content, /TPSL \/ 触发历史/)
  assert.doesNotMatch(content, /position-troubleshoot-mini-table__title">成交回报/)
  assert.doesNotMatch(content, /position-troubleshoot-mini-table__title">状态流转/)
})
...
test('PositionManagement.vue hoists subject workbench state while keeping recent decisions global', async () => {
  ...
  assert.match(subjectOverviewPanelSource, /<el-table-column label="持仓" min-width="116">/)
  assert.match(subjectOverviewPanelSource, /position_base_quantity/)
  assert.match(subjectOverviewPanelSource, /position_t_quantity/)
  ...
  assert.doesNotMatch(subjectOverviewPanelSource, /label="单笔止损触发"/)
  assert.doesNotMatch(subjectOverviewPanelSource, /label="全仓止损价"/)
})
...
test('position-management module doc reflects independent reconciliation panel and read-only audit semantics', async () => {
  ...
  assert.match(content, /右上工作区固定按 `持仓账本 -> 相关订单 -> 对账结果 -> 差异处理` 顺序展示/)
  assert.match(content, /持仓账本.*聚合买入列表/)
