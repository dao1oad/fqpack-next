...
test('PositionManagement.vue places rule matrix above inventory, uses the same metric-card typography, and widens inferred source columns', async () => {
  const content = await readFile(new URL('./PositionManagement.vue', import.meta.url), 'utf8')
  ...
  assert.match(content, /--position-symbol-limit-source-column-min-width:\s*240px;/)
  assert.match(content, /\.position-lower-grid\s*\{[\s\S]*align-items:\s*stretch;/)
  assert.match(content, /runtime-position-symbol-limit-ledger__grid\s*\{[\s\S]*minmax\(var\(--position-symbol-limit-source-column-min-width\),\s*1\.1fr\)\s+minmax\(var\(--position-symbol-limit-source-column-min-width\),\s*1\.1fr\)/)
...
test('position-management module doc reflects merged left panel, dirty-symbol filtering and truth-filled recent decisions', async () => {
  ...
  assert.match(content, /订单推断仓位 \/ stock_fills 仓位.*弹性宽列/)
