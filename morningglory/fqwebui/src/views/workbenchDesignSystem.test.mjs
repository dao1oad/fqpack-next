...
test('PositionManagement.vue consumes shared workbench page and panel primitives for state limits and decisions', () => {
  ...
  assert.match(positionSource, /<h1 class="workbench-page-title">仓位管理<\/h1>/)
...
test('GanttUnified.vue follows the shared research-page contract while keeping provider switch radio buttons', () => {
  ...
  assert.match(ganttSource, /import WorkbenchSummaryRow from ['"][^'"]*WorkbenchSummaryRow\.vue['"]/)
  assert.match(ganttSource, /import WorkbenchLedgerPanel from ['"][^'"]*WorkbenchLedgerPanel\.vue['"]/)
  assert.match(ganttSource, /import StatusChip from ['"][^'"]*StatusChip\.vue['"]/)
  assert.match(ganttSource, /<WorkbenchPage class="gantt-page">/)
  assert.match(ganttSource, /<MyHeader\s*\/>/)
  assert.match(ganttSource, /<WorkbenchToolbar class="gantt-toolbar">/)
  assert.match(ganttSource, /<div class="workbench-page-title">板块趋势<\/div>/)
  assert.match(ganttSource, /<div class="workbench-page-meta">[\s\S]*板块热点时间窗[\s\S]*支持 provider 切换与 drill-down/)
  assert.match(ganttSource, /<WorkbenchSummaryRow class="gantt-summary-row">/)
  assert.match(ganttSource, /<StatusChip variant="info">\s*provider <strong>\{\{\s*activeProviderLabel\s*\}\}<\/strong>\s*<\/StatusChip>/)
  assert.match(ganttSource, /<StatusChip variant="muted">\s*时间窗 <strong>\{\{\s*windowDays\s*\}\} 日<\/strong>\s*<\/StatusChip>/)
  assert.match(ganttSource, /<el-alert[\s\S]*class="workbench-alert gantt-page-alert"[\s\S]*type="info"[\s\S]*title="支持 provider 切换与 drill-down；主区保留原有趋势时间窗工作流。"/)
  assert.match(ganttSource, /<WorkbenchLedgerPanel class="gantt-history-panel">/)
  assert.match(ganttSource, /<div class="workbench-panel__title">板块趋势时间窗<\/div>/)
  assert.match(ganttSource, /<el-radio-group v-model="activeProvider"/)
  assert.doesNotMatch(ganttSource, /<div class="gantt-tabs">/)
})

test('GanttUnifiedStocks.vue follows the shared research-page contract while keeping provider switch radio buttons', () => {
  ...
  assert.match(ganttStocksSource, /import WorkbenchSummaryRow from ['"][^'"]*WorkbenchSummaryRow\.vue['"]/)
  assert.match(ganttStocksSource, /import WorkbenchLedgerPanel from ['"][^'"]*WorkbenchLedgerPanel\.vue['"]/)
  assert.match(ganttStocksSource, /import StatusChip from ['"][^'"]*StatusChip\.vue['"]/)
  assert.match(ganttStocksSource, /<WorkbenchPage class="gantt-page">/)
  assert.match(ganttStocksSource, /<MyHeader\s*\/>/)
  assert.match(ganttStocksSource, /<WorkbenchToolbar class="gantt-toolbar">/)
  assert.match(ganttStocksSource, /<div class="workbench-page-title">板块趋势 \/ 个股<\/div>/)
  assert.match(ganttStocksSource, /<div class="workbench-page-meta">[\s\S]*板块内个股回看[\s\S]*plateName \|\| plateKey \|\| '未选板块'/)
  assert.match(ganttStocksSource, /<WorkbenchSummaryRow class="gantt-summary-row">/)
  assert.match(ganttStocksSource, /<StatusChip variant="info">\s*provider <strong>\{\{\s*activeProviderLabel\s*\}\}<\/strong>\s*<\/StatusChip>/)
  assert.match(ganttStocksSource, /<StatusChip variant="muted">\s*时间窗 <strong>\{\{\s*windowDays\s*\}\} 日<\/strong>\s*<\/StatusChip>/)
  assert.match(ganttStocksSource, /<StatusChip :variant="plateKey \? 'success' : 'warning'">\s*当前板块 <strong>\{\{\s*plateName \|\| plateKey \|\| '未选板块'\s*\}\}<\/strong>\s*<\/StatusChip>/)
  assert.match(ganttStocksSource, /<el-alert[\s\S]*class="workbench-alert gantt-page-alert"[\s\S]*:type="plateKey \? 'info' : 'warning'"[\s\S]*:title="plateKey \? '当前页保留 provider 切换和板块内个股 drill-down。' : '请先从板块趋势页选择一个板块。'"/)
  assert.match(ganttStocksSource, /<WorkbenchLedgerPanel class="gantt-history-panel">/)
  assert.match(ganttStocksSource, /<div class="workbench-panel__title">板块内个股回看<\/div>/)
  assert.match(ganttStocksSource, /<div v-if="!plateKey" class="workbench-empty gantt-empty">请先从板块趋势页选择一个板块。<\/div>/)
...
test('SystemSettings.vue reuses shared workbench page toolbar and status chip primitives', () => {
  ...
  assert.match(systemSettingsSource, /<h1 class="workbench-page-title">系统设置<\/h1>/)
...
test('GanttShouban30Phase1.vue follows the shared research-page contract while keeping provider radio switch and workspace flow', () => {
  ...
  assert.match(shoubanSource, /import WorkbenchSummaryRow from ['"][^'"]*WorkbenchSummaryRow\.vue['"]/)
  ...
  assert.match(shoubanSource, /<MyHeader\s*\/>/)
  assert.match(shoubanSource, /<WorkbenchToolbar class="shouban30-toolbar">/)
  assert.match(shoubanSource, /<div class="workbench-page-title">首板筛选<\/div>/)
  assert.match(shoubanSource, /<div class="workbench-page-meta">[\s\S]*多 provider 首板观察[\s\S]*工作区联动 pre_pools \/ stock_pools/)
  assert.match(shoubanSource, /<WorkbenchSummaryRow class="shouban30-summary-row">/)
  assert.match(shoubanSource, /<el-alert[\s\S]*v-if="pageAlertVisible"[\s\S]*class="workbench-alert shouban30-page-alert"[\s\S]*:type="pageAlertType"[\s\S]*:title="pageAlertTitle"/)
  ...
  assert.match(shoubanSource, /<span class="workbench-panel__title">首板板块<\/span>/)
  assert.match(shoubanSource, /<span class="workbench-panel__title">热点标的<\/span>/)
  assert.match(shoubanSource, /<span class="workbench-panel__title">标的详情<\/span>/)
  assert.match(shoubanSource, /<span class="workbench-panel__title">工作区<\/span>/)
