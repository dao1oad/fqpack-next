...
test('ReconciliationWorkbench.vue consumes shared workbench panel and status chip primitives for unified audit and order troubleshooting', () => {
  assert.match(reconciliationSource, /import StatusChip from '\.\.\/components\/workbench\/StatusChip\.vue'/)
  assert.match(reconciliationSource, /import WorkbenchPage from '\.\.\/components\/workbench\/WorkbenchPage\.vue'/)
  assert.match(reconciliationSource, /import WorkbenchToolbar from '\.\.\/components\/workbench\/WorkbenchToolbar\.vue'/)
  assert.match(reconciliationSource, /import WorkbenchLedgerPanel from '\.\.\/components\/workbench\/WorkbenchLedgerPanel\.vue'/)
  assert.match(reconciliationSource, /import WorkbenchDetailPanel from '\.\.\/components\/workbench\/WorkbenchDetailPanel\.vue'/)
  assert.match(reconciliationSource, /<WorkbenchPage class="reconciliation-page">/)
  assert.match(reconciliationSource, /<WorkbenchToolbar class="reconciliation-toolbar">/)
  assert.match(reconciliationSource, /<WorkbenchLedgerPanel class="reconciliation-ledger-panel"[\s\S]*?>/)
  assert.match(reconciliationSource, /<WorkbenchDetailPanel class="reconciliation-detail-panel">/)
  assert.match(reconciliationSource, /<StatusChip variant="danger">\s*ERROR <strong>\{\{\s*summaryChips\.errorCount\s*\}\}<\/strong>/)
  assert.match(reconciliationSource, /<StatusChip variant="warning">\s*WARN <strong>\{\{\s*summaryChips\.warnCount\s*\}\}<\/strong>/)
  assert.match(reconciliationSource, /<StatusChip variant="success">\s*OK <strong>\{\{\s*summaryChips\.okCount\s*\}\}<\/strong>/)
  assert.match(reconciliationSource, /<div class="workbench-panel__actions reconciliation-panel__filters">[\s\S]*v-model="overviewFilters\.query"[\s\S]*v-model="overviewFilters\.auditStatus"[\s\S]*v-model="overviewFilters\.state"/)
  assert.match(reconciliationSource, /resolutionEndpointMissing/)
  assert.match(reconciliationSource, /当前运行中的后端未暴露 reconciliation-workspace 接口/)
  assert.match(reconciliationSource, /<el-tab-pane name="overview" label="概览"/)
  assert.match(reconciliationSource, /<el-tab-pane name="orders" label="相关订单"/)
  assert.match(reconciliationSource, /<el-tab-pane name="ledger" label="持仓账本"/)
  assert.match(reconciliationSource, /<el-tab-pane name="resolution" label="Resolution"/)
  assert.match(reconciliationSource, /class="reconciliation-ledger-workspace"/)
  assert.match(reconciliationSource, /class="reconciliation-ledger-side"/)
  assert.match(reconciliationSource, /reconciliation-ledger-side__top/)
  assert.match(reconciliationSource, /reconciliation-ledger-side__bottom/)
  assert.match(reconciliationSource, /prop="entry_short_id" label="entry_id"/)
  assert.match(reconciliationSource, /prop="entry_market_value_label" label="对应市值"/)
  assert.match(reconciliationSource, /prop="remaining_ratio_label" label="剩余百分比"/)
  assert.match(reconciliationSource, /prop="entry_slice_short_id" label="slice_id"/)
  assert.match(reconciliationSource, /prop="remaining_amount_label" label="剩余市值"/)
})
