...
test('buildWorkspaceTabs exposes must_pool actions for stock pool workspace', () => {
  ...
  assert.equal(mustPoolTab.sync_action_label, '同步到通达信')
  assert.equal(mustPoolTab.clear_action_label, '清空')
...
test('gantt tabs use 韭研公社 label and shouban30 keeps fixed workspace layout markers', async () => {
  ...
  assert.match(ganttContent, /<el-radio-group v-model="activeProvider"/)
  assert.match(ganttContent, /<el-radio-button value="xgb">选股通<\/el-radio-button>/)
  assert.match(ganttContent, /<el-radio-button value="jygs">韭研公社<\/el-radio-button>/)
  assert.doesNotMatch(ganttContent, /<el-tab-pane label="选股通" name="xgb" \/>/)
  assert.match(ganttStocksContent, /韭研公社/)
  assert.doesNotMatch(ganttStocksContent, /韭研公式/)
  assert.match(ganttStocksContent, /<el-radio-group v-model="activeProvider"/)
  assert.match(ganttStocksContent, /<el-radio-button value="xgb">选股通<\/el-radio-button>/)
  assert.match(ganttStocksContent, /<el-radio-button value="jygs">韭研公社<\/el-radio-button>/)
  assert.doesNotMatch(ganttStocksContent, /<el-tab-pane label="选股通" name="xgb" \/>/)
  assert.match(shoubanContent, /<WorkbenchToolbar class="shouban30-toolbar">/)
  assert.match(shoubanContent, /<WorkbenchSidebarPanel class="shouban30-panel shouban30-panel--plates">/)
  assert.match(shoubanContent, /<WorkbenchLedgerPanel class="shouban30-panel shouban30-panel--stocks">/)
  assert.match(shoubanContent, /<WorkbenchDetailPanel class="shouban30-panel shouban30-panel--detail">/)
  assert.match(shoubanContent, /<WorkbenchLedgerPanel class="shouban30-panel shouban30-panel--workspace">/)
  assert.match(shoubanContent, /<el-radio-group[\s\S]*:model-value="activeViewProvider"/)
  assert.match(shoubanContent, /<el-radio-button[\s\S]*v-for="option in VIEW_PROVIDER_OPTIONS"/)
  assert.doesNotMatch(shoubanContent, /<el-tabs[\s\S]*class="provider-tabs"/)
