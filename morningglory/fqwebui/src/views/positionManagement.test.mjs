...
test('PositionManagement view turns into the final dense workbench with selected-subject workspace in the right upper quadrant', () => {
  ...
  const middleColumnIndex = source.indexOf('position-workbench-column position-workbench-column--middle')
  ...
  assert.match(source, /reconciliationOverview/)
  assert.match(source, /PositionReconciliationPanel/)
  ...
  assert.match(source, /position-workbench-column > \.workbench-panel/)
  assert.match(source, /\.position-workbench-column--left,\s*[\s\S]*\.position-workbench-column--right\s*\{[\s\S]*grid-template-rows:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\)/)
