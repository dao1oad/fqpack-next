...
test('PositionManagement view merges runtime state and inventory into left panel and keeps only two top columns', () => {
  ...
  assert.match(source, /<div class="runtime-ledger__viewport">[\s\S]*v-for="row in ruleMatrix"/)
  assert.match(source, /<div class="runtime-ledger__viewport">[\s\S]*v-for="row in symbolLimitRows"/)
  assert.match(source, /<div class="runtime-ledger__viewport">[\s\S]*v-for="row in pagedDecisionRows"/)
  ...
  assert.match(source, /\.runtime-position-rule-ledger \.runtime-ledger__viewport \{[\s\S]*overflow:\s*visible;/)
