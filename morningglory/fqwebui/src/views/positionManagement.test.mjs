...
test('PositionManagement view merges runtime state and inventory into left panel and keeps only two top columns', () => {
  ...
  const reconciliationPanelIndex = source.indexOf('position-reconciliation-panel')
  ...
  assert.match(source, /<span>操作<\/span>\s*<span>当前来源<\/span>\s*<span>门禁<\/span>/)
  ...
  assert.doesNotMatch(source, /mismatchSymbolCount/)
  assert.doesNotMatch(source, /<span>一致性<\/span>/)
  assert.doesNotMatch(source, /<span>券商仓位<\/span>/)
  assert.doesNotMatch(source, /<span>账本仓位<\/span>/)
  assert.doesNotMatch(source, /<span>对账状态<\/span>/)
