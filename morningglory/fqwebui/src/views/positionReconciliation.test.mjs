...
test('PositionReconciliationPanel stays read-only and switches from audit cards to a dense reconciliation ledger', () => {
  assert.match(panelSource, /summaryRuleCards/)
  assert.match(panelSource, /summaryStateCards/)
  assert.match(panelSource, /position-reconciliation-ledger/)
  assert.match(panelSource, /position-reconciliation-ledger__grid/)
  assert.match(panelSource, /position-reconciliation-expanded/)
  assert.match(panelSource, /position-reconciliation-expanded__section/)
  assert.match(panelSource, />标的</)
  assert.match(panelSource, />结果</)
  assert.match(panelSource, />对账状态</)
  assert.match(panelSource, />latest resolution</)
  assert.match(panelSource, />signed gap</)
  assert.match(panelSource, />open gap</)
  assert.match(panelSource, />mismatch</)
  assert.match(panelSource, />关键视图</)
  assert.match(panelSource, /视图层证据表/)
  assert.match(panelSource, /规则检查表/)
  assert.match(panelSource, /差异说明表/)
  ...
  assert.doesNotMatch(panelSource, /position-audit-list/)
  assert.doesNotMatch(panelSource, /position-audit-row__headline/)
  assert.doesNotMatch(panelSource, /position-audit-preview-grid/)
  assert.doesNotMatch(panelSource, /position-audit-evidence__section/)
