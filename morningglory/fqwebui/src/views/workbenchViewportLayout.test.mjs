...
test('order, position, subject, tpsl and runtime pages no longer use page-level scrolling at desktop widths', () => {
  ...
  assert.match(positionSource, /\.position-reconciliation-panel,[\s\S]*\.position-subject-overview-host,[\s\S]*\.position-decision-panel \{[\s\S]*overflow:\s*hidden;/)
  assert.match(positionReconciliationSource, /\.position-audit-list \{[\s\S]*overflow:\s*auto;/)
