...
export function normalizeMetrics(raw: any): PerformanceMetrics {
  ...
  return {
    ...item,
    totalReturn: numberOrNull(item.total_return ?? item.totalReturn ?? item.mean_return),
    annualizedReturn: numberOrNull(item.annualized_return ?? item.annual_return ?? item.annualizedReturn),
    ...
    winRate: numberOrNull(item.win_rate ?? item.winRate),
    ...
    tradeCount: numberOrNull(item.trade_count ?? item.tradeCount),
