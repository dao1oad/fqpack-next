            ...
            if (total_count >= 4)
            {
                ...
                if (c >= 0)
                {
                    for (int n = c; n < length; n++)
                    {
                        ...
                        {
                            inner_result[n] = encode_signal(11, 1, signal);
                            break;
                        }
                    }
                }
...
REGISTER_CALC(11, F_S0011)

std::vector<int> F_S0011_ctx(
...
{
    return S0011_Calculator(high, low, open, close, vol, switch_opt, options, ctx).result();
}
