...
echo "CLX full artifact chain started at $(timestamp)"
...
run_governance_gate WI-004 v2-causal-signal-real
...
# 2. Materialize and deeply verify the event artifact.
if [[ ! -f "$event_dir/manifest.sha256" ]]; then
...
if [[ ! -f "$ranking_dir/manifest.sha256" ]]; then
...
for split in TRAIN VALIDATION HOLDOUT; do
  ...
  if [[ ! -f "$output/manifest.sha256" ]]; then
...
run_governance_gate WI-007 v2-portfolio-real
