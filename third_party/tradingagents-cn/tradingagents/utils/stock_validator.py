...
class StockDataPreparer:
            ...
            for data_source in priority_order:
                try:
                    if data_source == "baostock":
                        historical_records = await self._sync_baostock_single_stock(
                            stock_code,
                            start_date,
                            end_date
                        )
                        if historical_records > 0:
                            message = f"baostock synced historical records: {historical_records}"
                            return {
                                "success": True,
                                "message": message,
                        ...
                        last_error = f"{data_source}: historical sync failed"
                        continue
                    ...
                    try:
                        fin_result = await self._sync_financial_data_with_compat(service, stock_code,
    ...
    async def _sync_baostock_single_stock(self, stock_code: str, start_date: str, end_date: str) -> int:
        ...
        return await service._update_historical_data(stock_code, hist_data, "daily")

    async def _sync_financial_data_with_compat(self, service, stock_code: str, limit: int = 20) -> Dict:
        try:
        ...
        return await service.sync_financial_data(symbols=[stock_code])
